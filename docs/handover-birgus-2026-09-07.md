...
- Problema successivo risolto: `prisma db push` si fermava per data loss su residui Spedizioni (`shipment_statuses`, colonne `rows_shipments`/`columns_shipments`, enum `DocumentScope.SHIPMENT`) e `psql` falliva con `invalid URI query parameter: "schema"`. Il decommission Spedizioni e' stato eseguito una sola volta sul DB esistente. L'entrypoint non deve rilanciare quel cleanup a ogni boot: ora esegue solo `db:push`, `db:bootstrap`, `db:sync:workflow-tools` e avvio app.
...
- Se appaiono ancora warning Prisma su `shipment_*`, significa che il cleanup una tantum non e' stato eseguito sul DB puntato dall'ambiente corrente.
