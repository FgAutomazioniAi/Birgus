...
test("setup obtains the Garage version after role changes are staged", () => {
  assert.equal(
    nextGarageLayoutVersion("Current cluster layout version: 0"),
    "1",
  );
  assert.equal(
    nextGarageLayoutVersion("Current cluster layout version: 4"),
    "5",
  );
  assert.equal(
    nextGarageLayoutVersion("Staged cluster layout version: 7"),
    "7",
  );
  assert.equal(
    nextGarageLayoutVersion(
      "Run garage layout apply --version 9 to enact changes",
    ),
    "9",
  );
