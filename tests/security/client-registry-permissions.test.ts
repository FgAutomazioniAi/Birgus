...
test("company and client writes require registry write permission", () => {
  for (const method of [
    ...
    assert.deepEqual(
      permissionMetadata(NestProjectCrudController.prototype, method),
...
test("permanent registry deletion requires the developer-only permission", () => {
  assert.deepEqual(
    permissionMetadata(
      NestProjectCrudController.prototype,
      "permanentlyDeleteCompany",
    ),
    [PermissionKey.CLIENTS_DELETE_PERMANENTLY],
  );
  assert.deepEqual(
    permissionMetadata(
      NestProjectCrudController.prototype,
...
test("archive deletion is developer-only while restore keeps document write access", () => {
  assert.deepEqual(
    permissionMetadata(NestDocumentArchiveController.prototype, "restoreItem"),
    [PermissionKey.DOCUMENTS_WRITE],
  );
  assert.deepEqual(
    permissionMetadata(
      NestDocumentArchiveController.prototype,
      "permanentlyDeleteItem",
    ),
    [PermissionKey.CLIENTS_DELETE_PERMANENTLY],
  );
  assert.deepEqual(
    permissionMetadata(NestDocumentArchiveController.prototype, "emptyTrash"),
    [PermissionKey.CLIENTS_DELETE_PERMANENTLY],
  );
});
