...
test("PasswordHasher verifies the original password and rejects wrong values", async () => {
  ...
  assert.equal(
    await hasher.verifyPassword("Correct Horse Battery Staple", storedHash),
    true,
  );
  assert.equal(
    await hasher.verifyPassword("wrong-password", storedHash),
    false,
  );
});
...
test("PasswordHasher rejects malformed stored hashes", async () => {
  ...
  assert.equal(
    await hasher.verifyPassword("anything", "not-a-valid-hash"),
    false,
  );
});
