...
test("WorkflowRuntimeAccessPolicy validates every module used by a user-triggered run", async () => {
  ...
  await assert.rejects(
    policy.ensureRunAllowed({
      ...
      workflowModuleKey: "workflow_management",
      nodes: [
        {
          nodeKey: "ocr",
          tool: { moduleKey: "ddt_processing", enabled: true, deleted: false },
        },
      ],
    }),
    (error: unknown) =>
      error instanceof AppError && error.code === "MODULE_DISABLED",
  );
...
test("WorkflowRuntimeAccessPolicy checks workspace state for system runs and rejects disabled resources", async () => {
  ...
  assert.deepEqual(workspaceChecks, [
    "workflow_management",
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      ...
      error.code === "WORKFLOW_NODE_RESOURCE_DISABLED",
  );
});
