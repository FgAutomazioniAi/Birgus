...
test("agent session creation accepts the documented loaded-session response", async () => {
  const client = new BrainywareClient({
    ...
    fetchImpl: async (input) => {
      ...
      if (url.endsWith("/brainy/api/auth/service"))
        return jsonResponse({ token: "brainy-token" });
      if (url.endsWith("/brainy/api/ai/agents/agent-1/sessions"))
...
test("single-shot agent inference removes the temporary Brainyware session", async () => {
    ...
    fetchImpl: async (input, init) => {
      ...
      if (url.endsWith("/brainy/api/ai/agents/agent-1/run")) {
        ...
        return new Response(
          [
            "event: END",
            'data: {"session_id":"temporary-1","role":"agent","content":"Risposta"}',
            "",
          ].join("\n"),
          { headers: { "Content-Type": "text/event-stream" } },
        );
      }
      if (url.endsWith("/brainy/api/ai/agents/agent-1/sessions/temporary-1"))
        return new Response(null, { status: 204 });
      throw new Error(`Unexpected request: ${url}`);
    },
  });

  const result = await client.runAgentSingleShot({
    agentId: "agent-1",
    message: "Ciao",
  });
  ...
  assert.ok(
    requests.some((url) =>
      url.endsWith("/brainy/api/ai/agents/agent-1/sessions/temporary-1"),
    ),
  );
});
...
test("database inference uses the stateless Langflow bridge with empty history", async () => {
  ...
  const client = new BrainywareClient({
    ...
    fetchImpl: async (input, init) => {
      ...
      if (url.endsWith("/brainy/api/auth/service"))
        return jsonResponse({ token: "token-test" });
      if (url.endsWith("/brainy/api/database/connections/lf/list"))
        return jsonResponse({
          rows: [{ id: "42", name: "ERP", access_mode: "READ_ONLY" }],
        });
  ...
  assert.equal(
    requests[2].url,
    "https://brainyware.example/brainy/api/database/connections/lf/42/chat",
  );
...
test("model inference uses the OpenAI-compatible endpoint without sessions", async () => {
  ...
  assert.equal(
    requests[1].url,
    "https://brainyware.example/brainy/api/openai/v1/chat/completions",
  );
