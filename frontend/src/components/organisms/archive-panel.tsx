...
const knowledgeStatusLabel: Record<
  ActiveDocumentDto["knowledgeStatus"],
  string
> = {
...
export function ArchivePanel() {
  ...
  const [selectedPackage, setSelectedPackage] =
    useState<ArchivePackageKey>("complete");
  const [packages, setPackages] =
    useState<ArchivePackageSummary[]>(DEFAULT_PACKAGES);
  const [items, setItems] = useState<ArchivedItemDto[]>([]);
  const [selectedContainer, setSelectedContainer] =
    useState<ActiveDocumentContainer>("all");
  const [documentContainers, setDocumentContainers] = useState<
    ActiveDocumentContainerSummary[]
  >([]);
  ...
  const [pendingPermanentDelete, setPendingPermanentDelete] =
    useState<ArchivedItemDto | null>(null);
  const [roleKeys, setRoleKeys] = useState<string[]>([]);
  const canManageArchive = roleKeys.some((role) =>
  ...
  useEffect(() => {
    void fetch("/api/auth/session", { cache: "no-store" })
      ...
      .then((payload) =>
        setRoleKeys(
          (payload?.user?.roleKeys ?? []).map((role) =>
  ...
  const loadArchive = async () => {
    ...
    try {
      const response = await fetch(
        `/api/archive?package=${encodeURIComponent(selectedPackage)}`,
        {
          cache: "no-store",
        },
      );
  ...
  const loadDocuments = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/archive/documents?${search.toString()}`,
        { cache: "no-store" },
      );
      if (!response.ok) {
  ...
  const restoreItem = async (item: ArchivedItemDto) => {
    try {
      ...
      const response = await fetch(
        `/api/archive/${encodeURIComponent(item.entityType)}/${encodeURIComponent(item.entityId)}/restore`,
        {
          method: "POST",
        },
      );
      if (!response.ok) {
        const payload = (await response
          .json()
          .catch(() => ({ message: "Ripristino non riuscito." }))) as {
          message?: string;
        };
      ...
      toast.error(
        error instanceof Error ? error.message : "Ripristino non riuscito.",
      );
    } finally {
  ...
  const permanentlyDeleteItem = async (item: ArchivedItemDto) => {
    ...
    try {
      const response = await fetch(
        `/api/archive/${encodeURIComponent(item.entityType)}/${encodeURIComponent(item.entityId)}/permanent`,
        {
          method: "DELETE",
      ...
      if (!response.ok) {
        const payload = (await response.json().catch(() => ({
          message: "Eliminazione permanente non riuscita.",
        }))) as { message?: string };
        throw new Error(
          payload.message ?? "Eliminazione permanente non riuscita.",
        );
      }
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Eliminazione permanente non riuscita.",
      );
    } finally {
      setProcessingItemId(null);
    }
  ...
  const deleteDocument = async (document: ActiveDocumentDto) => {
    ...
    try {
      const response = await fetch(
        `/api/archive/documents/${encodeURIComponent(document.id)}`,
        { method: "DELETE" },
      );
      if (!response.ok) {
        const payload = (await response.json().catch(() => ({
          message: "Eliminazione documento non riuscita.",
        }))) as { message?: string };
        throw new Error(
          payload.message ?? "Eliminazione documento non riuscita.",
        );
      }
      toast.success(
        "Documento spostato nel cestino e rimosso dalla knowledge.",
      );
      await loadDocuments();
    } catch (error) {
      toast.error(
  ...
  const emptyTrash = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const payload = (await response
          .json()
          .catch(() => ({ message: "Svuotamento cestino non riuscito." }))) as {
          message?: string;
        };
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Svuotamento cestino non riuscito.",
      );
    } finally {
      setProcessingItemId(null);
    }
  ...
  return (
    <div className="w-full space-y-6">
      <header className="space-y-1">
        ...
        <Text variant="muted">
          {language === "en"
            ? "Documents and archived items in the workspace."
            : "Documenti ed elementi archiviati nel workspace."}
        </Text>
      </header>
        ...
        <>
          <div className="grid gap-3 md:grid-cols-3">
            {documentContainers.map((entry) => (
              <button
                key={entry.key}
                type="button"
                onClick={() => setSelectedContainer(entry.key)}
                className={cn(
                  "rounded-md border bg-bg-surface p-4 text-left transition-colors",
                  selectedContainer === entry.key
          ...
          <Card className="overflow-hidden">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border-subtle bg-bg-muted/60 px-4 py-3">
              ...
              <div className="flex flex-1 justify-end gap-2 sm:max-w-xl">
                <label className="relative min-w-40 flex-1">
                  <Search className="absolute left-2.5 top-2.5 h-4 w-4 text-text-muted" />
                  <input
                    value={documentQuery}
                    onChange={(event) => setDocumentQuery(event.target.value)}
                    className="h-9 w-full rounded-md border border-border-default bg-bg-surface pl-8 pr-3 text-sm text-text-primary outline-none focus:ring-2 focus:ring-ring-primary"
                    placeholder={t("archive.searchDocument")}
                  />
                </label>
                <select
                  ...
                  onChange={(event) =>
                    setKnowledgeFilter(
                      event.target.value as typeof knowledgeFilter,
                    )
                  }
                  className="h-9 rounded-md border border-border-default bg-bg-surface px-2 text-sm text-text-secondary outline-none focus:ring-2 focus:ring-ring-primary"
                >
                  <option value="all">{t("archive.knowledgeAll")}</option>
                  <option value="indexed">{t("archive.inKnowledge")}</option>
                  <option value="not_indexed">
                    {t("archive.notInKnowledge")}
                  </option>
                </select>
              </div>
            </div>
            {isLoading ? (
              <div className="flex items-center justify-center gap-2 p-10 text-text-muted">
                ...
                <span className="text-sm">{t("archive.loadingDocuments")}</span>
              </div>
            ) : documents.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-2 p-10 text-center">
                <FileText size={18} className="text-text-muted" />
                <Text variant="muted">{t("archive.noDocuments")}</Text>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-border-subtle text-sm">
                  <thead className="bg-bg-muted/40 text-left text-xs font-bold uppercase tracking-wide text-text-muted">
                    <tr>
                      <th className="px-4 py-3">{t("workflow.document")}</th>
                      <th className="px-4 py-3">{t("archive.origin")}</th>
                      <th className="px-4 py-3">Knowledge</th>
                      <th className="px-4 py-3">{t("archive.date")}</th>
                      <th className="px-4 py-3">{t("archive.size")}</th>
                  ...
                  <tbody className="divide-y divide-border-subtle/70 bg-bg-surface">
                    {documents.map((document) => (
                      <tr key={document.id}>
                        ...
                        <td className="px-4 py-3">
                          <span className="inline-flex rounded-full border border-border-default bg-bg-muted px-2 py-0.5 text-xs font-semibold text-text-secondary">
                            {document.container === "playgrounds"
                              ? t("archive.playgrounds")
                              : (document.moduleName ?? t("archive.workspace"))}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <span
                          ...
                          >
                            ...
                            {document.knowledgeStatus === "indexed"
                              ? t("archive.inKnowledge")
                              : document.knowledgeStatus === "processing"
                        ...
                        <td className="px-4 py-3 text-right">
                          <button
                            ...
                            aria-label={`${t("archive.moveToTrash")}: ${document.filename}`}
                            disabled={processingItemId === document.id}
                            onClick={() => void deleteDocument(document)}
                            className="rounded-md p-2 text-text-muted hover:bg-status-danger-bg hover:text-status-danger-text disabled:opacity-50"
                          >
        ...
        <>
          <div className="grid gap-3 md:grid-cols-3">
            {DEFAULT_PACKAGES.map((fallbackPackage) => {
              const entry =
                packageByKey.get(fallbackPackage.key) ?? fallbackPackage;
      ...
      <ConfirmDeleteDialog
        ...
        isBusy={
          ...
          processingItemId === pendingPermanentDelete.id
        }
        onCancel={() => setPendingPermanentDelete(null)}
        onConfirm={() =>
          pendingPermanentDelete
            ? permanentlyDeleteItem(pendingPermanentDelete)
            : undefined
        }
      />
    </div>
