...
const DEFAULT_CLIENT_COLUMN_ORDER: ClientColumnKey[] = [
  "name",
  "email",
  "phone",
  "notes",
...
const CLIENT_COLUMN_DEFS: ClientColumnDef[] = [
  ...
  },
  {
    ...
    render: (client) => (
      <div
        className="flex max-w-xs items-center gap-2 truncate text-sm italic text-text-muted"
        title={client.notes}
      >
...
export function ClientsTable() {
  ...
  const [activeRegistry, setActiveRegistry] = useState<"companies" | "clients">(
    "companies",
  );
  const [isLoading, setIsLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [isColumnsMenuOpen, setIsColumnsMenuOpen] = useState(false);
  const [columnOrder, setColumnOrder] = useState<ClientColumnKey[]>(
    DEFAULT_CLIENT_COLUMN_ORDER,
  );
  ...
  const [permanentDeleteTarget, setPermanentDeleteTarget] =
    useState<PermanentDeleteTarget | null>(null);
  const [isPermanentlyDeleting, setIsPermanentlyDeleting] = useState(false);
  const [companyForm, setCompanyForm] =
    useState<CompanyFormState>(EMPTY_COMPANY_FORM);
  const [clientForm, setClientForm] =
    useState<ClientFormState>(EMPTY_CLIENT_FORM);
  ...
  const canManageRegistry = roleKeys.some((role) =>
  ...
  const applyColumnConfig = (
    ...
    const parsedOrder = Array.isArray(config.order)
      ? config.order.filter((column): column is ClientColumnKey =>
          isClientColumnKey(String(column)),
        )
      : [];
    const uniqueOrder = Array.from(new Set(parsedOrder));
    const nextOrder = DEFAULT_CLIENT_COLUMN_ORDER.filter((column) =>
      uniqueOrder.includes(column),
    );
    const missing = DEFAULT_CLIENT_COLUMN_ORDER.filter(
      (column) => !nextOrder.includes(column),
    );
    setColumnOrder([...nextOrder, ...missing]);

    const parsedHidden = Array.isArray(config.hidden)
      ? config.hidden.filter((column): column is ClientColumnKey =>
          isClientColumnKey(String(column)),
        )
      : [];
    const optionalColumns = new Set(
      CLIENT_COLUMN_DEFS.filter((column) => !column.required).map(
        (column) => column.key,
      ),
    );
    setHiddenColumns(
      ...
      commit: async () => {
        ...
        if (!response.ok) {
          const payload = (await response
            .json()
            .catch(() => ({ message: "Errore archiviazione cliente" }))) as {
            message?: string;
          };
    ...
    scheduleUndoableAction({
      ...
      commit: async () => {
        ...
        if (!response.ok) {
          const payload = (await response
            .json()
            .catch(() => ({ message: "Errore archiviazione azienda" }))) as {
            message?: string;
          };
    ...
    try {
      ...
      if (!response.ok) {
        const payload = (await response.json().catch(() => ({
          message: "Eliminazione definitiva non riuscita",
        }))) as { message?: string };
        throw new Error(
          payload.message ?? "Eliminazione definitiva non riuscita",
        );
      }
      ...
      if (target.kind === "company") {
        ...
        setClients((current) =>
          current.map((client) =>
            client.companyId === target.item.id
  ...
  const firstVisibleRow = filteredClients.length
  ...
  const exportClientsPdf = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Errore durante l'esportazione clienti.",
      );
    } finally {
  ...
  const openCompanyEdit = (company: Company) => {
    ...
    setGeocodedAddress(
      company.latitude && company.longitude ? "Coordinate gia salvate." : "",
    );
    setGeocodingError("");
    setSelectedCompany(null);
    setIsCompanyDialogOpen(true);
  ...
  const saveCompany = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const payload = (await response
          .json()
          .catch(() => ({ message: "Errore salvataggio azienda" }))) as {
          message?: string;
        };
      ...
      setEditingCompanyId(null);
      toast.success(
        editingCompanyId ? "Azienda aggiornata." : "Azienda creata.",
      );
    } catch (error) {
      toast.error(
        error instanceof Error
          ? error.message
          : "Salvataggio azienda non riuscito.",
      );
    } finally {
  ...
  const geocodeCompany = async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Ricerca posizione non riuscita.";
  ...
  const saveClient = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const payload = (await response
          .json()
          .catch(() => ({ message: "Errore salvataggio cliente" }))) as {
          message?: string;
        };
      ...
      toast.success(
        editingClientId ? "Cliente aggiornato." : "Cliente creato.",
      );
    } catch (error) {
      toast.error(
        error instanceof Error
  ...
  const companyClients = (companyId: number) =>
  ...
  return (
    <div className="space-y-6">
                    ...
                    <div className="absolute right-0 top-12 z-20 w-80 rounded-[var(--radius-md)] border border-border-default bg-bg-surface p-3 shadow-elevated">
                      ...
                      <div className="mt-3 space-y-2">
                        {columnOrder.map((key, index) => {
                          const column = CLIENT_COLUMN_DEFS.find(
                            (item) => item.key === key,
                          );
                          if (!column) {
                            return null;
                      ...
                      <button type="button" className="text-left">
                        ...
                        <span className="block text-xs text-text-muted">
                          {company.isHeadquarters
                            ? "Sede principale"
                            : company.vatNumber || "Sede"}
                        </span>
                      </button>
                    </td>
                    <td className="px-6 py-4 text-sm text-text-secondary">
                      {companyClients(company.id).length}
                    </td>
                    <td className="px-6 py-4 text-sm text-text-secondary">
                      {company.email || "-"}
                    </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex justify-end gap-2">
                          <button
                            ...
                            className="rounded-[var(--radius-md)] p-1.5 text-brand-primary hover:bg-bg-subtle"
                          >
                            <Pencil size={18} />
                          </button>
                          <button
                            ...
                            className="rounded-[var(--radius-md)] p-1.5 text-status-danger-text hover:bg-status-danger-bg"
                          >
                            <Archive size={18} />
                          </button>
                        </div>
          ...
          <div className="flex flex-col items-start justify-between gap-3 border-t border-border-subtle p-4 sm:flex-row sm:items-center">
            <div className="flex flex-wrap items-center gap-3">
              <p className="text-xs text-text-muted">
                {firstVisibleRow} - {lastVisibleRow} su {filteredClients.length}{" "}
                clienti
              </p>
      ...
      {isCompanyDialogOpen ? (
        <RegistryDialog
          title={editingCompanyId ? "Modifica azienda" : "Nuova azienda"}
                ...
                >
                  ...
                  {field.multiline ? (
                    <textarea
                      ...
                      onChange={(event) =>
                        updateCompanyField(field.key, event.target.value)
                      }
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0">
                  ...
                  >
                    {geocodingError ||
                      geocodedAddress ||
                      "Completa indirizzo e citta, poi cerca la posizione."}
                  </p>
                </div>
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => void geocodeCompany()}
                  disabled={isGeocodingCompany || isSavingCompany}
                >
                  <MapPin size={16} />
      ...
      {isClientDialogOpen ? (
        <RegistryDialog
          title={editingClientId ? "Modifica cliente" : "Nuovo cliente"}
...
function RegistryDialog({
  ...
  return (
    ...
    >
      <div
        className="max-h-[86vh] w-full max-w-3xl overflow-hidden rounded-[var(--radius-lg)] border border-border-default bg-bg-surface shadow-elevated"
    ...
    <div className="grid gap-3 md:grid-cols-2">
      {items.map(([label, value]) => (
        <div
          key={label}
          className={cn(
            "rounded-[var(--radius-md)] border border-border-subtle bg-bg-page px-3 py-2",
            label === "Note" && "md:col-span-2",
          )}
        >
          <p className="text-xs font-bold uppercase text-text-muted">{label}</p>
          <p className="mt-1 whitespace-pre-wrap text-sm font-semibold text-text-primary">
            {value}
          </p>
        </div>
      ))}
