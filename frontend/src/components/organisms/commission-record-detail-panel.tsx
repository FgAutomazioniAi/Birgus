...
export function CommissionRecordDetailPanel({ id }: { id: string }) {
  ...
  const load = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`/api/commission-intake/records/${id}`, {
        cache: "no-store",
      });
      ...
      if (!response.ok || !payload.record)
        throw new Error(payload.message ?? "Impossibile caricare la commessa.");
      setRecord(payload.record);
    } catch (error) {
      toast.error(
        error instanceof Error
          ? error.message
          : "Impossibile caricare la commessa.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  if (!record)
    return (
      <Card className="p-5">
        <Text variant="muted">Commessa non disponibile.</Text>
      </Card>
    );
  return (
    <div className="space-y-5">
      <header className="flex flex-col gap-3 border-b border-border-subtle pb-3 md:flex-row md:items-start md:justify-between">
        <div>
          <Link
            href={APP_ROUTES.commissions}
            className="inline-flex h-9 items-center gap-2 text-sm font-bold text-text-secondary hover:text-text-primary"
          >
            <ArrowLeft size={16} />
            Torna alle commesse
          </Link>
          ...
          <p className="mt-1 text-sm text-text-muted">{record.code}</p>
        </div>
        {record.currentChecklistId ? (
          <Link href={APP_ROUTES.quotationChecklist(record.id)}>
            <Button>
      ...
      <Card className="p-5">
        ...
        <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-text-secondary">
          {record.description ?? "Nessuna descrizione inserita."}
        </p>
      </Card>
    </div>
  );
