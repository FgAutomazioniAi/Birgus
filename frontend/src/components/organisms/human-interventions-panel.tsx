...
export function HumanInterventionsPanel() {
  ...
  const loadItems = async () => {
    setIsLoading(true);
    try {
      const response = await fetch("/api/workflow-interventions?scope=mine", {
        cache: "no-store",
      });
  ...
  const decide = async (
    ...
    try {
      const response = await fetch(
        `/api/workflow-interventions/${id}/decision`,
        {
          method: "PATCH",
      ...
      if (!response.ok)
        throw new Error(payload.message ?? "Decisione non registrata.");
      setItems((current) => current.filter((item) => item.id !== id));
      setCount((current) => Math.max(0, current - 1));
      toast.success(
        "Decisione registrata. Il workflow riprende dal nodo successivo.",
      );
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Decisione non registrata.",
      );
    } finally {
  ...
  return (
    <>
      <Card className="flex items-center justify-between gap-4 p-4">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <Text className="font-semibold">Interventi da gestire</Text>
            <Text variant="caption">
              Decisioni umane che tengono in attesa un workflow.
            </Text>
          </div>
        </div>
        <Button
      ...
      {isOpen ? (
        <div
          className="fixed inset-0 z-[90] flex items-center justify-center bg-bg-overlay p-4"
          role="dialog"
          aria-modal="true"
          aria-label="Interventi da gestire"
          onMouseDown={(event) => {
          ...
          <div className="flex max-h-[85vh] w-full max-w-5xl flex-col overflow-hidden rounded-md border border-border-default bg-bg-surface shadow-elevated">
            <div className="flex items-center justify-between border-b border-border-default px-5 py-4">
              <div>
                ...
                <Text variant="caption">
                  Le decisioni riprendono il workflow senza ripetere gli step
                  precedenti.
                </Text>
              </div>
              <button
                ...
                className="rounded-md p-2 text-text-muted hover:bg-bg-muted"
                aria-label="Chiudi"
              >
                x
              </button>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-5">
              {isLoading ? (
                <Text variant="muted">Caricamento interventi...</Text>
              ) : null}
              {!isLoading && items.length === 0 ? (
                <Text variant="muted">Non ci sono interventi aperti.</Text>
              ) : null}
                ...
                {items.map((item) => (
                  <section key={item.id} className="py-4 first:pt-0">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      ...
                      <span
                        className={`text-xs font-bold uppercase ${priorityClass[item.priority]}`}
                      >
                        {item.priority}
                      </span>
                    </div>
                    <textarea
                      ...
                      placeholder="Nota decisione (facoltativa)"
                      rows={2}
                      className="mt-3 w-full rounded-md border border-border-default bg-bg-page px-3 py-2 text-sm text-text-primary outline-none focus:ring-2 focus:ring-ring-primary"
                    />
                    <div className="mt-3 flex flex-wrap gap-2">
                      <Button
                        size="sm"
                        variant="accent"
                        disabled={isDecidingId === item.id}
                      ...
                      <Button
                        ...
                        variant="outline"
                        disabled={isDecidingId === item.id}
                        onClick={() => void decide(item.id, "CHANGES_REQUIRED")}
                      >
                        Richiedi modifiche
                      </Button>
                      <Button
