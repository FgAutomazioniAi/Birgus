...
export function LoginForm({
  ...
  const [twoFactorSetupSecret, setTwoFactorSetupSecret] = useState<
    string | null
  >(null);
  const [twoFactorSetupUri, setTwoFactorSetupUri] = useState<string | null>(
    null,
  );
  const [twoFactorCode, setTwoFactorCode] = useState("");
  const [isVerifyingTwoFactor, setIsVerifyingTwoFactor] = useState(false);
  const [canRememberTrustedDevice, setCanRememberTrustedDevice] =
    useState(false);
  ...
  const [isChangingForcedPassword, setIsChangingForcedPassword] =
    useState(false);
  ...
  const generatePassword = () => {
    ...
    const randomIndex = (source: string) =>
    ...
    try {
      ...
      if (
        ...
        setTwoFactorSetupSecret(
          payload.setupRequired ? (payload.setup?.secret ?? null) : null,
        );
        setTwoFactorSetupUri(
          payload.setupRequired ? (payload.setup?.otpauthUri ?? null) : null,
        );
        setTwoFactorCode("");
  ...
  const handleVerifyTwoFactor = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/auth/login/2fa/verify", {
        ...
        body: JSON.stringify({
          ...
          rememberTrustedDevice:
            canRememberTrustedDevice && rememberTrustedDevice,
        }),
      });
      ...
      const payload = (await response
        .json()
        .catch(() => null)) as LoginApiSuccessPayload | null;
      ...
      const message =
        error instanceof Error ? error.message : "Verifica 2FA non riuscita.";
      toast.error(message);
    } finally {
  ...
  const handleForcedPasswordChange = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          payload?.message ?? "Aggiornamento password non riuscito.",
        );
      }
  ...
  const handleSendRecoveryCode = async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Invio codice non riuscito.";
      toast.error(message);
    } finally {
  ...
  return (
    <div
      data-theme={embedded ? "dark" : undefined}
      className={
        embedded
          ? "w-full"
          : "flex min-h-screen flex-col justify-center bg-bg-page px-4 py-12 sm:px-6 lg:px-8"
      }
    >
      {!embedded && (
      ...
      <div
        className={
          embedded
            ...
            : "mt-10 sm:mx-auto sm:w-full sm:max-w-md"
        }
      >
        <Card
          className={
            embedded
              ? "border-cyan-200/20 px-4 py-10 shadow-[0_20px_50px_rgba(0,0,0,0.35)] backdrop-blur-xs sm:px-10"
              : "border border-border-subtle px-4 py-10 shadow-elevated sm:px-10"
          }
          style={
            embedded ? { backgroundColor: "rgba(7, 16, 29, 0.48)" } : undefined
          }
        >
          {forcePasswordChange ? (
            <div className="space-y-5">
              <div>
                ...
                <p className="mt-1 text-sm text-text-secondary">
                  La password iniziale e temporanea. Scegline una personale per
                  proseguire.
                </p>
              </div>
              <div>
                ...
                <div className="relative mt-1">
                  <Input
                    ...
                    onChange={(event) =>
                      setForcedNewPassword(event.target.value)
                    }
                    autoComplete="new-password"
                    className="pr-20"
                    disabled={isChangingForcedPassword}
                  />
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setForcedConfirmPassword(event.target.value)
                  }
                  autoComplete="new-password"
                  className="mt-1"
                  disabled={isChangingForcedPassword}
              ...
              <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
                <div>
                  ...
                  <div className="relative mt-1">
                    ...
                    />
                    {errors.email && (
                      <p className="mt-1 text-xs font-medium text-status-danger-text">
