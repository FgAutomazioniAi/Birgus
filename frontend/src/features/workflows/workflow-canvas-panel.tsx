...
import { Badge, Button, Card, Input, Text } from "@/components/atoms";
...
function WorkflowActionsMenu({
  ...
  return (
    <div className="relative shrink-0">
      ...
      {isOpen ? (
        <div className="absolute right-0 top-full z-30 mt-1 w-44 rounded-[var(--radius-md)] border border-border-default bg-bg-surface p-1 shadow-elevated">
          {onSchedule ? (
            <button
              type="button"
              onClick={() => {
              ...
              className="flex w-full items-center gap-2 rounded-[var(--radius-sm)] px-2.5 py-2 text-left text-sm text-text-secondary hover:bg-bg-muted hover:text-text-primary"
            >
          ...
          {onDelete ? (
            <button
              ...
              className="flex w-full items-center gap-2 rounded-[var(--radius-sm)] px-2.5 py-2 text-left text-sm text-status-danger-text hover:bg-status-danger-bg"
            >
...
function resolveWorkflowKnowledgeMode(
  configuration: unknown,
): WorkflowKnowledgeMode {
  const config = toRecord(configuration);
  const contextPolicy = toRecord(config.contextPolicy);
  const value =
    config.knowledgeMode ??
    config.knowledge_mode ??
    contextPolicy.knowledgeMode ??
    contextPolicy.knowledge_mode;
  if (value === "saved") {
    return "hybrid";
  }
...
function normalizeNodeConfigurationForSave(
  node: DraftNode,
...
function isAiRequestTool(tool: WorkflowTool | null): boolean {
  ...
  return (
    tool.handlerKey.startsWith("langchain_orchestrator.") ||
...
function verificationFieldLabel(
...
function publicOutputOptions(
  source: DraftNode | undefined,
  sourceType: FlowNodeType | null,
): PublicOutputOption[] {
  if (!source) return [];
  if (sourceType === "verify-route") {
    const rules = Array.isArray(source.configuration.rules)
    ...
    return [
      ...
      ...rules.map((rule, index) => ({
        key: `field_${index + 1}`,
        label: verificationFieldLabel(toRecord(rule), index),
      })),
    ];
  }
  ...
  const defaults: Partial<Record<FlowNodeType, PublicOutputOption[]>> = {
    ...
    "brainyware-inference": [
      { key: "text", label: "Risposta Brainyware" },
      { key: "metadata", label: "Metadati inferenza" },
    ],
    "structure-data": [
      { key: "structured_data", label: "Dati estratti" },
      { key: "text", label: "Testo elaborato" },
    ],
    ...
    "compose-email": [
      { key: "subject", label: "Oggetto email" },
      { key: "text", label: "Corpo email" },
    ],
  };
  ...
  const fromSchema = Object.entries(properties).map(([key, schema]) => {
    const label =
      typeof toRecord(schema).title === "string"
        ? String(toRecord(schema).title)
        : "";
    return {
      key,
      label:
...
function FlowNodeCard({ data, selected }: NodeProps<Node<CanvasNodeData>>) {
  ...
  const [isConfirmingDefaultPrompt, setIsConfirmingDefaultPrompt] =
    useState(false);
  const Icon =
    NODE_KIND_ICONS[data.type] ??
    (data.paletteKind === "AGENT"
      ? Bot
      : data.paletteKind === "TOOL"
        ? Wrench
        : nodeKindIcon[data.kind]);
  const hasFieldTargets = [
    "input-knowledge",
    "document-set-ai",
    "llm",
    "brainyware-inference",
  ...
  const hasTarget =
    data.type !== "input-text" && data.type !== "schedule" && !hasFieldTargets;
  const hasSource =
    data.type !== "output" &&
  ...
  const scheduleRepeatEnabled =
    config.scheduleRepeatEnabled === undefined
      ? typeof config.scheduleRepeatValue === "string" &&
        config.scheduleRepeatValue.trim().length > 0
      : config.scheduleRepeatEnabled === true;
  const isFieldConnected = (field: string) =>
    data.incomingTargetHandles.includes(`field:${field}`) ||
    data.incomingTargetHandles.includes("__default__");
  const fieldInputClassName = (field: string) =>
    cn(
      ...
      isFieldConnected(field)
        ? "cursor-not-allowed bg-bg-muted text-text-muted"
        : "",
    );
  const dataHandleClassName =
    "!left-0 !h-3 !w-3 !border-2 !border-status-info-text !bg-status-info-text";
  // I connettori dei campi vivono nel contenuto imbottito del nodo: -12px li riallinea al bordo esterno.
  const textHandleClassName =
    "!left-[-12px] !h-3 !w-3 !border-2 !border-status-info-text !bg-status-info-text";
  const scheduleHandleClassName =
    "!left-[-12px] !h-3 !w-3 !border-2 !border-status-danger-text !bg-status-danger-text";
  const fileHandleClassName =
    "!left-[-12px] !h-3 !w-3 !border-2 !border-status-success-text !bg-status-success-text";
  const sourceFieldHandleClassName =
    "!right-[-12px] !h-3 !w-3 !border-2 !border-status-info-text !bg-status-info-text";
  ...
  const stringConfig = (key: string) =>
    typeof config[key] === "string" ? String(config[key]) : "";
  const boolConfig = (key: string) => config[key] === true;
  const patchConfig = (key: string, value: string) =>
    data.onConfigChange?.({ [key]: value });
  const patchCurrentPrompt = (value: string) =>
    data.onConfigChange?.({ instructions: value, currentPrompt: value });
  ...
  const updateRule = (index: number, patch: Record<string, unknown>) => {
    const nextRules = configuredRules.map((rule, ruleIndex) =>
      ruleIndex === index ? { ...rule, ...patch } : rule,
    );
    data.onConfigChange?.({ rules: nextRules });
  };
  const ruleIsConnected = (index: number) => isFieldConnected(`rule_${index}`);
  const ruleNeedsComparisonValue = (operator: unknown) =>
    !["not_empty", "exists"].includes(
      typeof operator === "string" ? operator : "not_empty",
    );
  const connectedFieldCount = (field: string) =>
    data.incomingTargetHandles.filter((handle) => handle === `field:${field}`)
      .length;
  const FieldHandle = ({
    field,
    title = "Collega valore",
  }: {
    field: string;
    title?: string;
  }) => (
    ...
    >
      {hasTarget ? (
        <Handle
          type="target"
          position={Position.Left}
          className={dataHandleClassName}
        />
      ) : null}
      <div className="relative mb-2 flex items-center gap-2">
        {["send-email", "send-telegram", "send-whatsapp"].includes(
          data.type,
        ) ? (
          <Handle
            type="target"
            id="control:schedule"
            position={Position.Left}
            className={scheduleHandleClassName}
            title="Collega Pianifica"
          />
        ) : null}
        <Icon className="h-4 w-4 text-text-primary" />
        <span className="text-sm font-medium text-text-primary">
          {localizedFlowNodeLabel(data.label, data.type, language)}
        </span>
      </div>
      ...
      {data.type === "input-text" ? (
        <div className="relative">
          ...
          <Handle
            ...
            className={sourceFieldHandleClassName}
            title="Usa il testo inserito"
          />
        </div>
      ) : null}

      {data.type === "input-file" ? (
        <div className="nodrag relative flex flex-col gap-1">
          <label
          ...
          <Handle
            ...
            position={Position.Right}
            className={sourceFieldHandleClassName
              .replace("status-info", "status-success")
              .replace("bg-status-info-text", "bg-status-success-text")}
            title="Usa il file caricato"
          />
        </div>
      ) : null}
      ...
      {data.type === "input-knowledge" ? (
        <div className="nodrag flex flex-col gap-1">
          <div className="relative">
            <FieldHandle field="query" title="Collega testo di ricerca" />
            <input
              id={fieldId("query")}
              name={fieldId("query")}
              type="text"
              ...
              disabled={isFieldConnected("query")}
              className={fieldInputClassName("query")}
            />
          </div>
          <select
            id={fieldId("category")}
      ...
      {data.type === "document-set-ai" ? (
        <div className="nodrag flex flex-col gap-1">
          <div className="relative">
            <FieldHandle
              field="documentIds"
              title="Collega identificativi dei documenti"
            />
            <textarea
              ...
              onChange={(event) =>
                patchConfig("documentIds", event.target.value)
              }
              placeholder={t("workflow.documentIds")}
              rows={3}
              disabled={isFieldConnected("documentIds")}
              className={fieldInputClassName("documentIds")}
            />
          </div>
          <div className="relative">
            <FieldHandle field="prompt" title="Collega richiesta di analisi" />
            <textarea
              id={fieldId("prompt")}
              name={fieldId("prompt")}
              value={currentPrompt}
              ...
              rows={2}
              disabled={isFieldConnected("prompt")}
              className={fieldInputClassName("prompt")}
            />
          </div>
          <WorkflowCheckbox
            ...
            onChange={(checked) =>
              data.onConfigChange?.({ use_deep_reasoning: checked })
            }
          />
        </div>
      ) : null}
        ...
        <div className="nodrag flex flex-col gap-2">
          <div className="relative">
            <Handle
              ...
              position={Position.Left}
              className={textHandleClassName}
              title="Collega contenuti da analizzare"
            />
            <label className="mb-1 block text-xs font-medium text-text-secondary">
              Contenuti da analizzare
              {connectedFieldCount("input_text") > 0
                ? ` (${connectedFieldCount("input_text")})`
                : ""}
            </label>
            <textarea
              ...
              onChange={(event) =>
                patchConfig("input_text", event.target.value)
              }
              placeholder={
                isFieldConnected("input_text")
                  ? "Contenuti collegati: verranno analizzati insieme"
                  : "Testo da analizzare"
              }
              rows={2}
        ...
        <div className="nodrag flex flex-col gap-2">
          <div className="relative">
            <Handle
              ...
              position={Position.Left}
              className={textHandleClassName}
              title="Collega il testo da elaborare"
            />
            <label className="mb-1 block text-xs font-medium text-text-secondary">
              Testo
              {connectedFieldCount("input_text") > 0
      ...
      {data.type === "structure-data" ? (
        <div className="nodrag flex flex-col gap-2">
          <div className="relative">
            <FieldHandle
              field="input_text"
              title="Collega testo da strutturare"
            />
            <textarea
              ...
              placeholder={
                isFieldConnected("input_text")
                  ? t("workflow.connectedInput")
                  : t("workflow.previousOutput")
              }
              rows={2}
              disabled={isFieldConnected("input_text")}
              className={fieldInputClassName("input_text")}
        ...
        <div className="nodrag flex flex-col gap-2">
          <div className="relative">
            {data.type === "generate-document" ? (
              <Handle
                ...
                position={Position.Left}
                className={textHandleClassName}
                title="Collega contenuto"
              />
            ) : null}
            <textarea
              id={fieldId(
                data.type === "quotation-docx"
                  ? "quotationDataJson"
                  : "content",
              )}
              name={fieldId(
              ...
              placeholder={
                data.type === "generate-document" && isFieldConnected("content")
                  ? t("workflow.connectedContent")
                  : data.type === "quotation-docx"
                    ? "Quotation JSON data"
                    : t("workflow.documentContent")
              }
              rows={3}
              disabled={
                data.type === "generate-document"
                  ? isFieldConnected("content")
                  : manualInputDisabled
              }
              className={
                data.type === "generate-document"
                  ? fieldInputClassName("content")
                  : inputClassName
              }
            />
          </div>
          <div className="relative">
            ...
            <input
              ...
              placeholder={
                data.type === "generate-document" &&
                isFieldConnected("file_name")
                  ? t("workflow.connectedFileName")
                  : t("workflow.fileName")
              }
              disabled={
                data.type === "generate-document" &&
          ...
          {data.type === "generate-document" ? (
            <>
              ...
              <WorkflowCheckbox
                ...
                onChange={(checked) =>
                  data.onConfigChange?.({ save_to_archive: checked })
                }
              />
            </>
          ) : null}
      ...
      {data.type === "send-email" ? (
        <div className="nodrag flex flex-col gap-1">
          <div className="relative">
            <FieldHandle field="to" title="Collega destinatario" />
            <input
              id={fieldId("to")}
              name={fieldId("to")}
              type="email"
              value={stringConfig("to")}
              onChange={(event) => patchConfig("to", event.target.value)}
              placeholder={t("workflow.emailRecipient")}
              disabled={isFieldConnected("to")}
              className={fieldInputClassName("to")}
            />
          </div>
          <div className="relative">
            <FieldHandle field="subject" title="Collega oggetto" />
            <input
              id={fieldId("subject")}
              name={fieldId("subject")}
              type="text"
              value={stringConfig("subject")}
              onChange={(event) => patchConfig("subject", event.target.value)}
              placeholder={t("workflow.subject")}
              disabled={isFieldConnected("subject")}
              className={fieldInputClassName("subject")}
            />
          </div>
          <div className="relative">
            <Handle
              ...
              position={Position.Left}
              className={textHandleClassName}
              title={t("workflow.connectContent")}
            />
            <textarea
              id={fieldId("text")}
              name={fieldId("text")}
              ...
              placeholder={
                isFieldConnected("text")
                  ? t("workflow.connectedContent")
                  : t("workflow.emailBody")
              }
              rows={3}
              disabled={isFieldConnected("text")}
              className={fieldInputClassName("text")}
            />
          </div>
        </div>
      ) : null}
          ...
          <TelegramChannelField
            ...
            onChange={(channelId) =>
              data.onConfigChange?.({
                ...
                chat_id: channelId ? "" : stringConfig("chat_id"),
              })
            }
        ...
        <div className="nodrag flex flex-col gap-1">
          <div className="relative">
            ...
            <input
              ...
              value={stringConfig("to")}
              onChange={(event) => patchConfig("to", event.target.value)}
              placeholder="Numero WhatsApp, es. 393..."
              disabled={isFieldConnected("to")}
              className={fieldInputClassName("to")}
            />
          </div>
        ...
        <div className="nodrag flex flex-col gap-1">
          <label className="flex flex-col gap-0.5 text-[10px] font-medium uppercase tracking-wide text-text-muted">
            ...
            <input
              ...
              onChange={(event) =>
                patchConfig("scheduleWhen", event.target.value)
              }
          ...
          <div className="flex items-end gap-2">
            <label className="flex min-w-0 flex-1 flex-col gap-0.5 text-[10px] font-medium uppercase tracking-wide text-text-muted">
              ...
              <div className="flex gap-1">
                <input
                  ...
                  onChange={(event) =>
                    patchConfig("scheduleRepeatValue", event.target.value)
                  }
                  placeholder="7"
                ...
                <select
                  ...
                  onChange={(event) =>
                    patchConfig("scheduleRepeatUnit", event.target.value)
                  }
                  disabled={!scheduleRepeatEnabled}
                  className="w-1/2 rounded-md border border-border-default bg-bg-page px-2 py-1 text-xs normal-case tracking-normal text-text-primary outline-none focus:ring-2 focus:ring-ring-primary"
                >
            ...
            <WorkflowCheckbox
              checked={scheduleRepeatEnabled}
              label="Ripeti"
              onChange={(checked) =>
                data.onConfigChange?.({ scheduleRepeatEnabled: checked })
              }
            />
          </div>
        </div>
      ...
      {data.type === "compose-email" ? (
        <div className="nodrag flex flex-col gap-1">
          <div className="relative">
            <FieldHandle field="context" title="Collega contesto" />
            <textarea
              id={fieldId("context")}
              name={fieldId("context")}
              value={stringConfig("context")}
              onChange={(event) => patchConfig("context", event.target.value)}
              placeholder={
                isFieldConnected("context")
                  ? "Input dal nodo collegato"
                  : "Di cosa deve parlare l'email?"
              }
              rows={2}
              disabled={isFieldConnected("context")}
              className={fieldInputClassName("context")}
            />
          </div>
          <label
          ...
          >
            ...
            <select
              id={fieldId("tone")}
              name={fieldId("tone")}
              value={stringConfig("tone") || "professionale"}
              onChange={(event) => {
                const tone = event.target.value;
                const prompts: Record<string, string> = {
                  professionale:
                    "Scrivi in modo chiaro, cortese e operativo, senza risultare rigido.",
                  formale:
                    "Usa un registro formale e rispettoso, con formule di apertura e chiusura appropriate.",
                  informale:
                    "Scrivi in modo cordiale e diretto, adatto a una relazione di lavoro confidenziale.",
                  breve:
                    "Scrivi un messaggio molto breve e concreto, senza preamboli superflui.",
                };
                data.onConfigChange?.({
                  tone,
                  instructions: prompts[tone] ?? prompts.professionale,
                  currentPrompt: prompts[tone] ?? prompts.professionale,
                });
              }}
          ...
          <textarea
            id={fieldId("extra_instructions")}
            name={fieldId("extra_instructions")}
            value={currentPrompt}
            onChange={(event) => patchCurrentPrompt(event.target.value)}
      ...
      {data.type === "format-text-ai" ||
      data.type === "format-text-template" ? (
        <div className="nodrag flex flex-col gap-2">
          <div className="relative">
            <Handle
              type="target"
            ...
            <textarea
              ...
              rows={2}
              disabled={isFieldConnected("content")}
              className={fieldInputClassName("content")}
            />
          </div>
          <div className="relative">
            <Handle
              ...
              position={Position.Left}
              className={textHandleClassName}
              title="Collega template documento"
            />
            <textarea
              id={fieldId("template")}
              name={fieldId("template")}
              value={stringConfig("template")}
              onChange={(event) => patchConfig("template", event.target.value)}
              placeholder={
                isFieldConnected("template")
                  ? "Template dal nodo collegato"
                  : data.type === "format-text-template"
                    ? "Template con {{content}} o {{fields...}}"
                    : "Template documento"
              }
              rows={3}
              disabled={isFieldConnected("template")}
              className={fieldInputClassName("template")}
            />
          </div>
          {data.type === "format-text-ai" ? (
            <textarea
              id={fieldId("format-instructions")}
              name={fieldId("format-instructions")}
              value={currentPrompt}
              onChange={(event) => patchCurrentPrompt(event.target.value)}
              placeholder="Istruzioni aggiuntive"
              rows={2}
      ...
      {data.type === "verify-route" ? (
        <div className="nodrag flex flex-col gap-2">
          {configuredRules.map((rule, index) => (
            <div
              key={`${data.nodeId}-rule-${index}`}
              className="relative rounded-md border border-border-subtle bg-bg-muted p-1.5"
            >
              <Handle
                type="target"
                id={`field:rule_${index}`}
                position={Position.Left}
                className={textHandleClassName}
                title={`Collega valore alla regola ${index + 1}`}
              />
              <div
                className={cn(
                  "mb-1 flex min-h-7 items-center rounded-md border px-2 text-xs font-medium",
                  ruleIsConnected(index)
                    ? "border-border-default bg-bg-page text-text-primary"
                    : "border-dashed border-border-default bg-bg-muted text-text-muted",
                )}
              >
                {ruleIsConnected(index)
                  ? `Controlla: ${data.incomingFieldLabels[`rule_${index}`] ?? "Output collegato"}`
                  : "Collega un valore in ingresso"}
              </div>
              <div className="grid grid-cols-[minmax(0,1fr)_24px] gap-1">
                <select
                  ...
                  className="min-w-0 rounded-md border border-border-default bg-bg-page px-1 py-1 text-xs text-text-primary outline-none focus:ring-2 focus:ring-ring-primary"
                >
                ...
                <button
                  type="button"
                  className="rounded-md px-1 text-text-muted hover:bg-bg-page hover:text-status-danger-text"
              ...
              {ruleNeedsComparisonValue(rule.operator) ? (
                typeof rule.operator === "string" &&
                rule.operator === "between" ? (
                  <div className="mt-1 grid grid-cols-2 gap-1">
                    <input
                      ...
                      onChange={(event) =>
                        updateRule(index, { min: event.target.value })
                      }
                      placeholder="Minimo"
                      className="min-w-0 rounded-md border border-border-default bg-bg-page px-2 py-1 text-xs text-text-primary outline-none focus:ring-2 focus:ring-ring-primary"
                    />
                    <input
                      value={
                      ...
                      onChange={(event) =>
                        updateRule(index, { max: event.target.value })
                      }
                      placeholder="Massimo"
                      className="min-w-0 rounded-md border border-border-default bg-bg-page px-2 py-1 text-xs text-text-primary outline-none focus:ring-2 focus:ring-ring-primary"
                    />
                  ...
                  <input
                    ...
                    className="mt-1 w-full min-w-0 rounded-md border border-border-default bg-bg-page px-2 py-1 text-xs text-text-primary outline-none focus:ring-2 focus:ring-ring-primary"
                  />
                )
              ) : null}
            </div>
          ...
          <div className="relative flex min-h-8 items-center justify-center">
            <button
              type="button"
              className="w-36 rounded-md border border-border-default px-2 py-1 text-xs font-semibold text-text-secondary hover:bg-bg-muted"
              onClick={() =>
                data.onConfigChange?.({
                  rules: [
                    ...configuredRules,
                    {
                      label: `Valore ${configuredRules.length + 1}`,
                      operator: "not_empty",
                      value: "",
                    },
                  ],
            ...
            <span className="pointer-events-none absolute right-3 top-0.5 text-[10px] font-bold text-text-primary">
              {language === "en" ? "T" : "V"}
            </span>
            <span className="pointer-events-none absolute right-3 bottom-0.5 text-[10px] font-bold text-text-primary">
              F
            </span>
          </div>
        </div>
      ...
      {data.type === "verify-route" ? (
        <>
          <Handle
            ...
            style={{ top: "auto", bottom: "23px" }}
            title={language === "en" ? "Route when true" : "Instrada se valido"}
          />
          <Handle
            type="source"
            id="invalid"
            ...
            style={{ top: "auto", bottom: "7px" }}
            title={
              language === "en" ? "Route when false" : "Instrada se non valido"
            }
          />
        </>
      ) : null}
      ...
      {data.type === "human-review" || data.type === "request-decision" ? (
        <div className="nodrag flex flex-col gap-1">
          <div className="relative">
            <FieldHandle field="title" title="Collega titolo" />
            <input
              id={fieldId("title")}
              name={fieldId("title")}
              value={stringConfig("title")}
              onChange={(event) => patchConfig("title", event.target.value)}
              placeholder={
                data.type === "request-decision"
                  ? "Decisione richiesta"
                  : "Titolo revisione"
              }
              disabled={isFieldConnected("title")}
              className={fieldInputClassName("title")}
            />
          </div>
          <div className="relative">
            <FieldHandle field="message" title="Collega messaggio" />
            <textarea
              id={fieldId("message")}
              name={fieldId("message")}
              value={stringConfig("message")}
              onChange={(event) => patchConfig("message", event.target.value)}
              placeholder={
                data.type === "request-decision"
                  ? "Cosa deve decidere l'utente?"
                  : "Motivo della revisione"
              }
              rows={2}
              disabled={isFieldConnected("message")}
              className={fieldInputClassName("message")}
            />
          </div>
          {data.type === "request-decision" ? (
            <select
              ...
              value={stringConfig("priority") || "normal"}
              onChange={(event) => patchConfig("priority", event.target.value)}
              className="w-full rounded-md border border-border-default bg-bg-page px-2 py-1 text-xs text-text-primary outline-none focus:ring-2 focus:ring-ring-primary"
            >
          ...
          <input
            ...
            name={fieldId("assigneeUserId")}
            value={stringConfig("assigneeUserId")}
            onChange={(event) =>
              patchConfig("assigneeUserId", event.target.value)
            }
            placeholder="ID utente assegnatario (facoltativo)"
            className="w-full rounded-md border border-border-default bg-bg-page px-2 py-1 text-xs text-text-primary outline-none focus:ring-2 focus:ring-ring-primary"
          />
        </div>
      ) : null}
          ...
          >
            ...
            <ChevronDown
              className={cn(
                ...
                isDefaultPromptOpen ? "rotate-180" : "",
              )}
            />
          </button>
          {isDefaultPromptOpen ? (
          ...
          {isConfirmingDefaultPrompt ? (
            <div className="absolute bottom-full left-0 z-20 mb-2 w-64 rounded-md border border-border-default bg-bg-surface p-3 shadow-elevated">
              <p className="text-xs font-semibold text-text-primary">
                {t("workflow.updateDefaultPrompt")}
              </p>
              <p className="mt-1 text-xs text-text-muted">
                {t("workflow.updateDefaultPromptHint")}
              </p>
              <div className="mt-3 flex justify-end gap-2">
                <button
                  type="button"
                  className="rounded-md px-2 py-1 text-xs font-medium text-text-muted hover:bg-bg-muted"
                  onClick={() => setIsConfirmingDefaultPrompt(false)}
                >
                  Annulla
                </button>
                <button
                  type="button"
                  className="rounded-md bg-brand-primary px-2 py-1 text-xs font-semibold text-text-inverse hover:bg-brand-primary-hover"
                  onClick={confirmDefaultPromptChange}
                >
                  Conferma
                </button>
              </div>
      ...
      {hasSource && data.type !== "verify-route" ? (
        <Handle
          type="source"
          position={Position.Right}
...
export function WorkflowCanvasPanel() {
  ...
  const [knowledgeMode, setKnowledgeMode] =
    useState<WorkflowKnowledgeMode>("on_demand");
  ...
  const [workflowLockError, setWorkflowLockError] = useState<string | null>(
    null,
  );
  ...
  const [openWorkflowActionsMenuId, setOpenWorkflowActionsMenuId] = useState<
    string | null
  >(null);
  const [workflowToSchedule, setWorkflowToSchedule] =
    useState<WorkflowSummary | null>(null);
  const [scheduleStartsAt, setScheduleStartsAt] = useState("");
  const [scheduleEvery, setScheduleEvery] = useState("1");
  const [scheduleUnit, setScheduleUnit] = useState<"hours" | "days" | "weeks">(
    "days",
  );
  const [isSavingSchedule, setIsSavingSchedule] = useState(false);
  const [canConfigureSchedules, setCanConfigureSchedules] = useState(false);
  ...
  const [outputPreviewNodeId, setOutputPreviewNodeId] = useState<string | null>(
    null,
  );
  const [expandedResultCard, setExpandedResultCard] = useState<string | null>(
    null,
  );
  const [openToolbarGroup, setOpenToolbarGroup] = useState<string>(
    TOOLBAR_GROUPS[0].id,
  );
  const [nodePickerGroupId, setNodePickerGroupId] = useState<string | null>(
    null,
  );
  ...
  const [uploadedFiles, setUploadedFiles] = useState<
    Record<string, UploadedWorkflowFile>
  >({});
  ...
  const selectedAgent = selectedNode?.moduleAgentId
    ? (agentById.get(selectedNode.moduleAgentId) ?? null)
    : null;
  const selectedTool = selectedNode?.moduleToolId
    ? (toolById.get(selectedNode.moduleToolId) ?? null)
    : null;
  const activeToolbarGroup =
    TOOLBAR_GROUPS.find((group) => group.id === openToolbarGroup) ??
    TOOLBAR_GROUPS[0];
  const nodePickerGroup =
    TOOLBAR_GROUPS.find((group) => group.id === nodePickerGroupId) ?? null;
  const featuredToolbarItems = activeToolbarGroup.items.slice(0, 4);
  const hasMoreToolbarItems =
    activeToolbarGroup.items.length > featuredToolbarItems.length;
  ...
  const nodePickerItems = (nodePickerGroup?.items ?? []).filter((kind) => {
    ...
    return (
      !query ||
      flowNodeLabel(kind, language).toLowerCase().includes(query) ||
      flowNodeDescription(kind, language).toLowerCase().includes(query)
    );
  });
  ...
  const playgroundCard = useMemo<ModuleCard>(() => {
    const playground =
      workflows.find((item) => item.key === PLAYGROUND_KEY) ?? null;
    return {
      cardKey: "playground",
  ...
  const incomingFieldLabelsByTargetId = useMemo(() => {
    ...
    for (const edge of draftEdges.filter(
      (item) =>
        item.isEnabled && /^field:rule_\d+$/.test(item.targetHandle ?? ""),
    )) {
      const source = draftNodes.find(
        (node) => node.clientId === edge.sourceClientId,
      );
      const sourceType = source
        ? inferFlowNodeType(
            source,
            source.moduleToolId
              ? (toolById.get(source.moduleToolId) ?? null)
              : null,
          )
        : null;
      ...
      const selectedOutputKey =
        typeof condition.selected_output_key === "string"
          ? String(condition.selected_output_key)
          : "";
      const selected =
        options.find((option) => option.key === selectedOutputKey) ??
        options[0];
      const targetLabels = labels.get(edge.targetClientId) ?? {};
      targetLabels[(edge.targetHandle ?? "").replace("field:", "")] =
        typeof condition.input_label === "string" &&
        condition.input_label.trim()
          ? condition.input_label.trim()
          : (selected?.label ?? "Output collegato");
  ...
  const personalWorkflows = useMemo(
    () =>
      workflows.filter(
        (item) =>
          item.moduleKey === "workflow_management" &&
          item.key !== PLAYGROUND_KEY,
      ),
    [workflows],
  );
  const availableAgents: WorkflowAgent[] = [];
  const agentTools = useMemo(() => tools.filter(isAgentTool), [tools]);
  const advancedLangChainTools = useMemo(
    () => tools.filter(isAdvancedLangChainTool),
    [tools],
  );
    ...
    if (!workflow || isPlaygroundWorkflow) {
      return tools.filter(
        (tool) =>
          ...
          tool.handlerKey !==
            "document_intelligence.search_workspace_knowledge",
      );
    }
    return tools.filter(
      (tool) =>
        ...
        tool.handlerKey !==
          "document_intelligence.search_workspace_knowledge" &&
        (tool.moduleKey === workflow.moduleKey ||
          tool.moduleKey === "document_intelligence"),
    );
  }, [isPlaygroundWorkflow, tools, workflow]);
  const canEditSelectedAgentPrompt = Boolean(
    selectedAgent &&
      workflow &&
      !isPlaygroundWorkflow &&
      selectedAgent.moduleKey === workflow.moduleKey,
  );
  const workflowReadOnly = Boolean(
    workflow && (!workflowLock || workflowLockError),
  );
  ...
  const loadCatalog = useCallback(async () => {
    setIsLoading(true);
    try {
      const [workflowsResponse, toolsResponse, agentsResponse] =
        await Promise.all([
      ...
      toast.error(
        error instanceof Error ? error.message : "Errore caricamento workflow.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const loadWorkflow = useCallback(
    async (workflowId: string) => {
      ...
      const nextDraftNodes = detail.nodes.map((item) => {
        ...
        const agent = node.moduleAgentId
          ? (agents.find((candidate) => candidate.id === node.moduleAgentId) ??
            null)
          : null;
        const tool = node.moduleToolId
          ? (tools.find((candidate) => candidate.id === node.moduleToolId) ??
            null)
          : null;
        ...
        return {
          ...
          configuration: normalizeAiPromptConfiguration(
            node.configuration,
            agent?.activePrompt ||
              currentPromptFromConfiguration(toRecord(tool?.configuration)) ||
              defaultPromptForTool(tool),
            agent?.originalPrompt ?? defaultPromptForTool(tool),
          ),
        };
      });
      const idToClientId = new Map(
        nextDraftNodes.map((node) => [node.id ?? node.nodeKey, node.clientId]),
      );
      const nextDraftEdges = detail.edges.map((edge) =>
        toDraftEdge(edge, idToClientId),
      );
      setWorkflow(detail);
  ...
  useEffect(() => {
    ...
    const heartbeat = async () => {
      try {
        const response = await fetch(
          `/api/workflows/${workflow.id}/lock/heartbeat`,
          { method: "POST" },
        );
        ...
        if (!response.ok || !payload.lock) {
          ...
          if (workflowLockFailureCountRef.current >= 3)
            reportLockLoss(
              payload.message ?? "Il workflow è ora in sola lettura.",
            );
          return;
        }
        workflowLockFailureCountRef.current = 0;
        ...
        workflowLockFailureCountRef.current += 1;
        if (workflowLockFailureCountRef.current >= 3)
          reportLockLoss("Il workflow è ora in sola lettura.");
      }
    };
    const acquire = async () => {
      try {
        const response = await fetch(`/api/workflows/${workflow.id}/lock`, {
          method: "POST",
        });
        const payload = (await response.json().catch(() => ({}))) as {
          lock?: { id: string };
        ...
        if (!response.ok || !payload.lock)
          throw new Error(
            payload.message ?? "Workflow in modifica da un altro utente.",
          );
        if (!cancelled) {
    ...
    return () => {
      ...
      void fetch(`/api/workflows/${workflow.id}/lock/release`, {
        method: "POST",
        keepalive: true,
      });
    };
  ...
  const patchNodeConfiguration = useCallback(
  ...
  const patchEdgeCondition = useCallback(
    (clientId: string, conditionPayload: Record<string, unknown> | null) => {
      setDraftEdges((current) =>
        current.map((edge) =>
          edge.clientId === clientId ? { ...edge, conditionPayload } : edge,
        ),
      );
    },
    [],
  ...
  const selectFieldOutput = useCallback(
    (edge: DraftEdge, outputKey: string, outputLabel: string) => {
      const existingInputLabel =
        typeof toRecord(edge.conditionPayload).input_label === "string"
          ? String(toRecord(edge.conditionPayload).input_label).trim()
          : "";
      setDraftEdges((current) =>
        current.map((item) => {
      ...
      setDraftNodes((current) =>
        current.map((node) => {
          ...
          const rules = Array.isArray(node.configuration.rules)
            ? node.configuration.rules.map(toRecord)
            : [];
          if (!rules[ruleIndex]) return node;
          rules[ruleIndex] = {
            ...rules[ruleIndex],
            label: existingInputLabel || outputLabel,
          };
  ...
  const setFieldInputLabel = useCallback(
    (edge: DraftEdge, inputLabel: string, fallbackLabel: string) => {
      ...
      setDraftNodes((current) =>
        current.map((node) => {
          ...
          rules[ruleIndex] = {
            ...rules[ruleIndex],
            label: normalized || fallbackLabel,
          };
          return { ...node, configuration: { ...node.configuration, rules } };
        }),
      );
  ...
  const handleNodeFileChange = useCallback(
    async (clientId: string, file: File) => {
      if (file.size > MAX_WORKFLOW_UPLOAD_BYTES) {
        toast.error(
          "File troppo grande per il Playground workflow. Limite: 15 MB.",
        );
        return;
      }
  ...
  useEffect(() => {
    ...
    agentById,
    draftEdges,
    draftNodeStructureKey,
    handleNodeFileChange,
    incomingFieldLabelsByTargetId,
    incomingHandlesByTargetId,
    patchNodeConfiguration,
    setEdges,
    setNodes,
    toolById,
  ...
  const createPlayground = async () => {
    const existingPlayground = workflows.find(
      (item) => item.key === PLAYGROUND_KEY,
    );
    ...
    try {
      const response = await fetch("/api/workflows", {
        ...
        body: JSON.stringify({
          ...
          label: "Playground",
          description:
            "Workflow libero per testare combinazioni di agent e tool.",
          isEnabled: true,
          isDefault: false,
      ...
      if (!response.ok) {
        ...
        throw new Error(
          typeof payload?.message === "string"
            ? payload.message
            : "Creazione Playground non riuscita.",
        );
      }
      ...
      toast.error(
        error instanceof Error ? error.message : "Errore creazione Playground.",
      );
    } finally {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          typeof payload?.message === "string"
            ? payload.message
            : "Creazione workflow non riuscita.",
        );
      }
      const created = (await response.json()) as WorkflowDetail;
      ...
      toast.error(
        error instanceof Error ? error.message : "Errore creazione workflow.",
      );
    } finally {
  ...
  const copyPersonalWorkflow = async (source: WorkflowSummary) => {
    try {
      const detailResponse = await fetch(`/api/workflows/${source.id}`, {
        cache: "no-store",
      });
      if (!detailResponse.ok)
        throw new Error("Impossibile leggere il workflow da copiare.");
      const detail = (await detailResponse.json()) as WorkflowDetail;
      const nodeKeyById = new Map(
        detail.nodes.map((node) => [node.id, node.nodeKey]),
      );
      const key = `playground_${Date.now().toString(36)}`;
      const response = await fetch("/api/workflows", {
        method: "POST",
      ...
      toast.error(
        error instanceof Error ? error.message : "Copia workflow non riuscita.",
      );
    }
  };
  ...
  const deletePersonalWorkflow = async (item: WorkflowSummary) => {
    try {
      const response = await fetch(`/api/workflows/${item.id}`, {
        method: "DELETE",
      });
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Eliminazione workflow non riuscita.",
      );
    }
  };

  const openScheduleDialog = (item: WorkflowSummary) => {
    const defaultStart = new Date(Date.now() + 5 * 60_000);
    defaultStart.setSeconds(0, 0);
    setScheduleStartsAt(defaultStart.toISOString().slice(0, 16));
    setScheduleEvery("1");
    setScheduleUnit("days");
    setWorkflowToSchedule(item);
  };

  const createWorkflowSchedule = async () => {
    if (!workflowToSchedule || isSavingSchedule) return;
    const start = new Date(scheduleStartsAt);
    const interval = Number(scheduleEvery);
    const multiplier =
      scheduleUnit === "hours"
        ? 3_600
        : scheduleUnit === "days"
    ...
    if (
      ...
      toast.error("Inserisci una data e una frequenza valide.");
      return;
    }
    try {
      setIsSavingSchedule(true);
      const response = await fetch(
        `/api/workflows/${workflowToSchedule.id}/schedules`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            startsAt: start.toISOString(),
            repeatEverySeconds: interval * multiplier,
          }),
        },
      );
      const payload = (await response.json().catch(() => ({}))) as {
      ...
      if (!response.ok)
        throw new Error(
          payload.message ?? "Impossibile creare la schedulazione.",
        );
      setWorkflowToSchedule(null);
      toast.success("Workflow pianificato.");
    } catch (error) {
      toast.error(
        error instanceof Error
          ? error.message
          : "Impossibile creare la schedulazione.",
      );
    } finally {
  ...
  const addAgentNode = (
    ...
    commitHistory();
    const nextIndex =
      draftNodes.filter((item) => item.nodeKind === "AGENT").length + 1;
  ...
  const addToolNode = (
    ...
    commitHistory();
    const nextIndex =
      draftNodes.filter((item) => item.nodeKind === "TOOL").length + 1;
  ...
  const addFlowNode = (
    ...
    if (kind === "input-text" || kind === "input-file") {
      ...
      const node: DraftNode = {
        ...
        configuration:
          kind === "input-text" ? { promptText: "" } : { documentUrl: "" },
        isEnabled: true,
        isRequired: false,
      };
      setDraftNodes((current) => [...current, node]);
    ...
    if (!tool) {
      toast.info(
        language === "en"
          ? `Backend tool is not configured for ${flowNodeLabel(kind, language)}.`
          : `Tool backend non configurato per ${flowNodeLabel(kind, language)}.`,
      );
      return;
    }
  ...
  const handleDeleteSelected = () => {
    ...
    commitHistory();
    setDraftNodes((current) =>
      current.filter((node) => !selectedNodeIds.has(node.clientId)),
    );
    setDraftEdges((current) =>
      current.filter(
        (edge) =>
  ...
  const patchSelectedNode = (patch: Partial<DraftNode>) => {
    ...
    setDraftNodes((current) =>
      current.map((item) =>
        item.clientId === selectedNode.clientId ? { ...item, ...patch } : item,
      ),
    );
  };
  ...
  const savePrompt = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Errore salvataggio prompt.",
      );
    } finally {
  ...
  const resetPrompt = async () => {
    ...
    try {
      const response = await fetch(
        `/api/agents/${selectedAgent.id}/reset-prompt`,
        { method: "POST" },
      );
      if (!response.ok) {
      ...
      toast.error(
        error instanceof Error ? error.message : "Errore reset prompt.",
      );
    } finally {
      setIsSavingPrompt(false);
    }
  ...
  const saveWorkflow = async (options?: {
    ...
    try {
      ...
      const nodeKeyByClientId = new Map(
        draftNodes.map((node) => [node.clientId, node.nodeKey]),
      );
      const payloadEdges = draftEdges.map((edge, index) => ({
        ...(edge.id && uuidPattern.test(edge.id) ? { id: edge.id } : {}),
        sourceNodeKey:
          nodeKeyByClientId.get(edge.sourceClientId) ?? edge.sourceClientId,
        targetNodeKey:
          nodeKeyByClientId.get(edge.targetClientId) ?? edge.targetClientId,
        sourceHandle: edge.sourceHandle ?? null,
        targetHandle: edge.targetHandle ?? null,
        label: edge.label ?? null,
        conditionPayload: edge.conditionPayload ?? null,
      ...
      const semanticNode = (node: (typeof payloadNodes)[number]) =>
        JSON.stringify({
          nodeKey: node.nodeKey,
          nodeKind: node.nodeKind,
          label: node.label,
      ...
      const storedNodeByKey = new Map(
        workflow.nodes.map((node) => [node.nodeKey, node]),
      );
      const hasNodeChanges =
        payloadNodes.length !== workflow.nodes.length ||
        payloadNodes.some((node) => {
      ...
      const storedEdges = workflow.edges.map((edge) => ({
        sourceNodeKey:
          workflow.nodes.find((node) => node.id === edge.sourceNodeId)
            ?.nodeKey ?? edge.sourceNodeId,
        targetNodeKey:
          workflow.nodes.find((node) => node.id === edge.targetNodeId)
            ?.nodeKey ?? edge.targetNodeId,
        sourceHandle: edge.sourceHandle ?? null,
        targetHandle: edge.targetHandle ?? null,
        label: edge.label ?? null,
        conditionPayload: edge.conditionPayload ?? null,
      ...
      const hasEdgeChanges =
        JSON.stringify(payloadEdges.map(({ id: _id, ...edge }) => edge)) !==
        JSON.stringify(storedEdges);
      ...
      if (!response.ok) {
        ...
        throw new Error(
          typeof payload?.message === "string"
            ? payload.message
            : "Salvataggio workflow non riuscito.",
        );
      }
      ...
      toast.error(
        error instanceof Error ? error.message : "Errore salvataggio workflow.",
      );
      return null;
    } finally {
      setIsSaving(false);
  ...
  const exportWorkflow = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : t("workflow.exportFailed"),
      );
    }
  };
  ...
  const importWorkflow = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : t("workflow.importFailed"),
      );
    }
  };
    ...
    try {
      ...
      const workflowFiles = Object.fromEntries(
        draftNodes
          ...
          .filter((item): item is [string, UploadedWorkflowFile] =>
            Boolean(item),
          ),
      );
      const inputPayload =
        Object.keys(workflowFiles).length > 0
  ...
  const pollRun = async (runId: string) => {
    for (let attempt = 0; attempt < 40; attempt += 1) {
      ...
      if (["COMPLETED", "FAILED", "CANCELED"].includes(run.status)) {
        if (run.status === "FAILED") {
          toast.error(
            run.errorMessage || "L'esecuzione del workflow non e' riuscita.",
          );
    ...
    toast.warning(
      "L'esecuzione e' ancora in corso: controlla i risultati tra qualche istante.",
    );
  };
  ...
  const onNodesChange = useCallback(
    (changes: NodeChange<Node<CanvasNodeData>>[]) => {
      ...
      if (removedIds.size > 0) {
        setDraftNodes((current) =>
          current.filter((draft) => !removedIds.has(draft.clientId)),
        );
        setDraftEdges((current) =>
          current.filter(
            (edge) =>
              !removedIds.has(edge.sourceClientId) &&
              !removedIds.has(edge.targetClientId),
          ),
        );
  ...
  const onNodeDragStop = useCallback<OnNodeDrag<Node<CanvasNodeData>>>(
    (_, node) => {
      if (workflowReadOnly) return;
      commitHistory();
      setDraftNodes((current) =>
  ...
  const onConnect = useCallback(
    (connection: Connection) => {
      ...
      const source = draftNodes.find(
        (node) => node.clientId === connection.source,
      );
      const target = draftNodes.find(
        (node) => node.clientId === connection.target,
      );
      const sourceType = source
        ? inferFlowNodeType(
            source,
      ...
      const targetType = target
        ? inferFlowNodeType(
            target,
            target.moduleToolId
              ? (toolById.get(target.moduleToolId) ?? null)
              : null,
          )
        : null;
      if (
        ...
        (!["send-email", "send-telegram", "send-whatsapp"].includes(
          ...
          connection.targetHandle !== "control:schedule")
      ) {
        toast.error(
          "Pianifica va collegato al connettore di pianificazione dei nodi Resoconto.",
        );
        return;
      }
      commitHistory();
      ...
      if (
        targetType === "verify-route" &&
        /^field:rule_\d+$/.test(connection.targetHandle ?? "")
      ) {
        const defaultOutput = publicOutputOptions(source, sourceType)[0];
        if (defaultOutput) {
          const ruleIndex = Number(
            (connection.targetHandle ?? "").replace("field:rule_", ""),
          );
          setDraftNodes((current) =>
            current.map((node) => {
              ...
              return {
                ...node,
                configuration: { ...node.configuration, rules },
              };
            }),
          );
        }
      ...
      setEdges((current) =>
        addEdge({ ...connection, id: edge.clientId, animated: true }, current),
      );
    },
  ...
  const isValidConnection = useCallback(
    (connection: Connection | Edge) => {
      ...
      if (!source || !target || source.clientId === target.clientId)
        return false;
      const sourceType = inferFlowNodeType(
        source,
        source.moduleToolId
          ? (toolById.get(source.moduleToolId) ?? null)
          : null,
      );
      const targetType = inferFlowNodeType(
        target,
        target.moduleToolId
          ? (toolById.get(target.moduleToolId) ?? null)
          : null,
      ...
      return (
        ["send-email", "send-telegram", "send-whatsapp"].includes(targetType) &&
        connection.targetHandle === "control:schedule"
      );
    },
    [draftNodes, toolById],
  ...
  const onEdgesChange = useCallback(
    (changes: EdgeChange<Edge>[]) => {
      ...
      if (removedIds.size > 0) {
        setDraftEdges((current) =>
          current.filter((edge) => !removedIds.has(edge.clientId)),
        );
      }
    },
  ...
  const onPaletteDrop = useCallback(
    (event: DragEvent<HTMLDivElement>) => {
      ...
      const payload = parsePaletteDragPayload(
        event.dataTransfer.getData("application/x-birgus-workflow-node"),
      );
      if (!payload) {
        return;
      }
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              ...
              <PageHelpHint text={t("workflow.help")} />
            </div>
        ...
        <section>
          <div className="mb-3 flex items-baseline justify-between gap-3">
            ...
            <Text variant="caption">
              {t("workflow.activeModules", { count: moduleCards.length })}
            </Text>
          </div>
          {moduleCards.length > 0 ? (
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {moduleCards.map((card) => (
                <div
                  key={card.cardKey}
                  className="relative min-h-44 rounded-[var(--radius-md)] border border-border-default bg-bg-surface shadow-card transition hover:border-brand-primary hover:bg-bg-subtle"
                >
                  <button
                    className="h-full w-full p-5 text-left disabled:cursor-not-allowed disabled:opacity-60"
                  ...
                  {canConfigureSchedules && card.workflow ? (
                    <div className="absolute right-3 top-3">
                      <WorkflowActionsMenu
                        isOpen={openWorkflowActionsMenuId === card.cardKey}
                        onToggle={() =>
                          setOpenWorkflowActionsMenuId((current) =>
                            current === card.cardKey ? null : card.cardKey,
                          )
                        }
                        onClose={() => setOpenWorkflowActionsMenuId(null)}
                        onSchedule={() => openScheduleDialog(card.workflow!)}
                      />
                    </div>
                  ) : null}
                </div>
        ...
        <section className="border-t border-border-subtle pt-4">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <Text variant="caption">
                {t("workflow.freeWorkflows", {
                  count:
                    personalWorkflows.length +
                    (playgroundCard.workflow ? 1 : 0),
                })}
              </Text>
            </div>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              ...
              <div className="flex items-start justify-between gap-3">
                ...
                >
                  ...
                  <p className="mt-4 truncate text-sm font-semibold text-text-primary">
                    {playgroundCard.title}
                  </p>
                  <p className="mt-1 line-clamp-2 text-xs text-text-muted">
                    {playgroundCard.description}
                  </p>
                </button>
                {playgroundCard.workflow ? (
                  <WorkflowActionsMenu
                    isOpen={
                      openWorkflowActionsMenuId === playgroundCard.workflow.id
                    }
                    onToggle={() =>
                      setOpenWorkflowActionsMenuId((current) =>
                        current === playgroundCard.workflow!.id
                          ? null
                          : playgroundCard.workflow!.id,
                      )
                    }
                    onClose={() => setOpenWorkflowActionsMenuId(null)}
                    onSchedule={
                      canConfigureSchedules
                        ? () => openScheduleDialog(playgroundCard.workflow!)
                        : undefined
                    }
                    onCopy={() =>
                      void copyPersonalWorkflow(playgroundCard.workflow!)
                    }
                    onDelete={() =>
                      setWorkflowToDelete(playgroundCard.workflow!)
                    }
                  />
                ) : null}
              </div>
            ...
            {personalWorkflows.map((item) => (
              <div
                key={item.id}
                className="min-h-32 rounded-[var(--radius-md)] border border-border-default bg-bg-surface p-4 shadow-card transition hover:border-brand-primary"
              >
                <div className="flex items-start justify-between gap-3">
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <p className="mt-1 line-clamp-2 text-xs text-text-muted">
                      {item.description || "Workflow libero"}
                    </p>
                  </button>
                  <WorkflowActionsMenu
                    isOpen={openWorkflowActionsMenuId === item.id}
                    onToggle={() =>
                      setOpenWorkflowActionsMenuId((current) =>
                    ...
                    onCopy={() => void copyPersonalWorkflow(item)}
                    onDelete={() => setWorkflowToDelete(item)}
                  />
                </div>
              </div>
            ))}
          ...
          >
            <Card className="w-full max-w-md p-5 shadow-elevated">
              ...
              <div className="mt-5 flex justify-end gap-2">
                ...
                <Button
                  onClick={() => void createPersonalWorkflow()}
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] min-h-[640px] flex-col overflow-hidden">
      <input
        ...
        type="file"
        accept="application/json,.json,.birgus-workflow.json"
        className="hidden"
        ...
        <div className="flex flex-wrap gap-2">
          <label className="flex items-center gap-2 rounded-md border border-border-default bg-bg-page px-2 py-1 text-xs font-medium text-text-secondary">
            ...
            <select
              ...
              value={knowledgeMode}
              onChange={(event) =>
                setKnowledgeMode(event.target.value as WorkflowKnowledgeMode)
              }
      ...
      <div className="border-b border-border-default bg-bg-page px-6 pt-2">
        <div
          ...
          role="tablist"
          aria-label={t("workflow.nodeCategories")}
        >
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? NODE_CATEGORY_TAB_ACTIVE[group.category]
                    : "border-transparent text-text-muted hover:bg-bg-muted hover:text-text-primary",
                )}
              >
                {isBrainyware ? (
                  <img
                    src="/brainyware-logo.png"
                    alt=""
                ...
                {isBrainyware ? (
                  <span className="inline-flex min-w-5 items-center justify-center rounded-full bg-gradient-to-r from-amber-400 via-red-500 via-purple-600 to-blue-500 px-1.5 py-0.5 text-xs font-bold text-white shadow-sm">
                    {group.items.length}
                  </span>
                ) : (
                  <Badge tone={NODE_CATEGORY_BADGE_TONE[group.category]}>
                    {group.items.length}
                  </Badge>
                )}
              </button>
      ...
      <div
        className="border-b border-border-default bg-bg-surface px-6 py-3"
        role="tabpanel"
      >
      ...
      {selectedEdge
        ? (() => {
            ...
            const sourceNode = draftNodes.find(
              (node) => node.clientId === selectedEdge.sourceClientId,
            );
            const targetNode = draftNodes.find(
              (node) => node.clientId === selectedEdge.targetClientId,
            );
            const sourceType = sourceNode
              ? inferFlowNodeType(
                  sourceNode,
                  sourceNode.moduleToolId
                    ? (toolById.get(sourceNode.moduleToolId) ?? null)
                    : null,
                )
              : null;
            const targetType = targetNode
              ? inferFlowNodeType(
                  targetNode,
                  targetNode.moduleToolId
                    ? (toolById.get(targetNode.moduleToolId) ?? null)
                    : null,
                )
              : null;
            const isVerificationInput =
              targetType === "verify-route" &&
              /^field:rule_\d+$/.test(selectedEdge.targetHandle ?? "");
            const isFieldInput =
              selectedEdge.targetHandle?.startsWith("field:") === true;
            const selectedOutputKey =
              typeof condition.selected_output_key === "string"
            ...
            const selectedOutput =
              outputOptions.find(
                (option) => option.key === selectedOutputKey,
              ) ?? outputOptions[0];
            const hasCondition = Object.keys(condition).length > 0;
            const defaultPath = sourceNode
              ? `outputs.${sourceNode.nodeKey}.valid`
              : "outputs.node.output";
            return (
              <div className="flex flex-wrap items-end gap-3 border-b border-border-default bg-bg-page px-6 py-3 text-sm">
                <div className="min-w-0 flex-1">
                  <p className="font-semibold text-text-primary">
                    Instradamento del collegamento
                  </p>
                  <p className="truncate text-xs text-text-muted">
                    {sourceNode?.label ?? "Nodo origine"} -&gt;{" "}
                    {targetNode?.label ?? "Nodo destinazione"}
                  </p>
                </div>
                {isFieldInput ? (
                  <>
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setFieldInputLabel(
                            selectedEdge,
                            event.target.value,
                            selectedOutput?.label ?? "Output collegato",
                          )
                        }
                        placeholder={
                          selectedOutput?.label ?? "Output collegato"
                  ...
                  <>
                    <label className="flex items-center gap-2 text-text-secondary">
                      <input
                        ...
                        onChange={(event) =>
                          patchEdgeCondition(
                            selectedEdge.clientId,
                            event.target.checked
                              ? { path: defaultPath, operator: "truthy" }
                              : null,
                          )
                        }
                      />
                    ...
                    {hasCondition ? (
                      <p className="text-xs text-text-muted">
                        Le condizioni avanzate esistenti restano attive. Per una
                        verifica leggibile usa Verifica e instrada.
                      </p>
                    ) : null}
                  </>
                )}
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        ...
        <RunResultsPanel
          ...
          onToggleCollapsed={() =>
            setIsRunsPanelCollapsed((current) => !current)
          }
          onToggleCard={(cardId) =>
            setExpandedResultCard((current) =>
      ...
      <WorkflowOutputDialog
        ...
        latestRun={latestRun}
        nodeKey={
          draftNodes.find((node) => node.clientId === outputPreviewNodeId)
            ?.nodeKey ?? null
        }
        nodeLabel={
          draftNodes.find((node) => node.clientId === outputPreviewNodeId)
            ?.label ?? flowNodeLabel("output", language)
        }
      ...
      {workflowToSchedule ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="workflow-schedule-title"
          onMouseDown={(event) => {
            if (event.target === event.currentTarget && !isSavingSchedule)
              setWorkflowToSchedule(null);
          }}
        >
          <Card className="w-full max-w-md p-5 shadow-elevated">
            <div className="flex items-start justify-between gap-4">
              <div>
                <Text as="h2" variant="h2" id="workflow-schedule-title">
                  Pianifica workflow
                </Text>
                <Text variant="caption" className="mt-1 block">
                  {workflowToSchedule.label}
                </Text>
              </div>
              <button
                type="button"
                disabled={isSavingSchedule}
                onClick={() => setWorkflowToSchedule(null)}
                className="rounded-[var(--radius-sm)] p-1.5 text-text-muted hover:bg-bg-muted hover:text-text-primary"
                title="Chiudi"
              >
                <X className="h-4 w-4" />
            ...
            <label
              className="mt-5 block text-sm font-semibold text-text-primary"
              htmlFor="workflow-schedule-start"
            >
              Primo avvio
              <Input
                id="workflow-schedule-start"
                type="datetime-local"
            ...
            <div className="mt-4 grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-3">
              <label
                ...
                htmlFor="workflow-schedule-every"
              >
                Ripeti ogni
                <Input
                  id="workflow-schedule-every"
                  type="number"
                  min={1}
                  value={scheduleEvery}
                  onChange={(event) => setScheduleEvery(event.target.value)}
                  className="mt-1.5"
                />
              </label>
              <label
                className="text-sm font-semibold text-text-primary"
                htmlFor="workflow-schedule-unit"
              >
                Unita
                <select
                  id="workflow-schedule-unit"
                  value={scheduleUnit}
                  onChange={(event) =>
                    setScheduleUnit(
                      event.target.value as "hours" | "days" | "weeks",
                    )
                  }
                  className="mt-1.5 h-11 w-full rounded-[var(--radius-md)] border border-border-default bg-bg-page px-3 text-sm text-text-primary focus-visible:border-brand-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring-primary"
                >
                  <option value="hours">ore</option>
                  <option value="days">giorni</option>
            ...
            <div className="mt-5 flex justify-end gap-2">
              ...
              <Button
                disabled={isSavingSchedule}
                onClick={() => void createWorkflowSchedule()}
              >
                {isSavingSchedule ? "Pianificazione..." : "Pianifica"}
              </Button>
            </div>
          </Card>
        </div>
      ...
      <BirgusDialog
        ...
        onConfirm={() => {
          ...
          if (workflow) void deletePersonalWorkflow(workflow);
        }}
      />
      {nodePickerGroup ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="workflow-node-picker-title"
          onMouseDown={(event) => {
            if (event.target === event.currentTarget)
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <Text
                  ...
                  variant="h2"
                  className="text-lg"
                  id="workflow-node-picker-title"
                >
                  {language === "en"
                    ? nodePickerGroup.titleEn
                    : nodePickerGroup.title}
                </Text>
                <Text variant="muted">
                  {t("workflow.nodesAvailable", {
                    count: nodePickerGroup.items.length,
                  })}
                </Text>
              </div>
              <button
                type="button"
                onClick={() => setNodePickerGroupId(null)}
                className="rounded-md p-1 text-text-muted hover:bg-bg-muted hover:text-text-primary"
                aria-label={t("workflow.closeNodePicker")}
              >
                <X className="h-5 w-5" />
              </button>
            </div>
