...
export const moduleDescriptions: Record<string, string> = {
  project_management:
    "OCR, strutturazione preventivo, generazione DOCX e invio.",
  ddt_processing: "OCR, analisi DDT e indicizzazione del risultato.",
  measure_report:
    "Analisi AI dei report misurazioni e persistenza righe fuori tolleranza.",
  workflow_management: "Area libera per provare combinazioni di agent e tool.",
  document_intelligence:
    "OCR, knowledge refresh e ricerca sui documenti indicizzati.",
};
...
const moduleDescriptionsEn: Record<string, string> = {
  project_management:
    "OCR, quotation structuring, document generation, and delivery.",
  ddt_processing: "OCR, DDT analysis, and result indexing.",
  measure_report:
    "AI analysis of measurement reports and persistence of out-of-tolerance rows.",
  workflow_management: "A free area to test agent and tool combinations.",
  document_intelligence:
    "OCR, knowledge refresh, and search across indexed documents.",
};
...
const NODE_KIND_DESCRIPTIONS: Record<
  FlowNodeType,
  Record<UiLanguage, string>
> = {
  "input-text": {
    it: "Inserisce testo manuale nel flusso.",
    en: "Adds manual text to the flow.",
  },
  "input-file": {
    it: "Carica un file da usare nel flusso.",
    en: "Uploads a file for the flow.",
  },
  "input-knowledge": {
    it: "Cerca nella knowledge del workspace.",
    en: "Searches workspace knowledge.",
  },
  "document-set-ai": {
    it: "Analizza uno o più documenti selezionati.",
    en: "Analyzes one or more selected documents.",
  },
  ocr: {
    it: "Estrae testo da documenti e immagini.",
    en: "Extracts text from documents and images.",
  },
  llm: {
    it: "Genera una risposta con l'agente AI.",
    en: "Generates an answer with the AI agent.",
  },
  "brainyware-inference": {
    it: "Interroga Brainyware senza creare una chat persistente.",
    en: "Queries Brainyware without creating a persistent chat.",
  },
  "structure-data": {
    it: "Trasforma il testo in dati strutturati.",
    en: "Transforms text into structured data.",
  },
  "generate-document": {
    it: "Crea un documento scaricabile.",
    en: "Creates a downloadable document.",
  },
  "quotation-docx": {
    it: "Genera un preventivo in formato documento.",
    en: "Generates a quotation document.",
  },
  "check-mailbox": {
    it: "Legge nuovi messaggi dalla casella email.",
    en: "Reads new messages from the mailbox.",
  },
  "send-email": {
    it: "Invia un messaggio email.",
    en: "Sends an email message.",
  },
  "send-telegram": {
    it: "Invia un messaggio Telegram.",
    en: "Sends a Telegram message.",
  },
  "send-whatsapp": {
    it: "Invia un messaggio WhatsApp.",
    en: "Sends a WhatsApp message.",
  },
  schedule: {
    it: "Avvia un invio programmato.",
    en: "Starts a scheduled delivery.",
  },
  "compose-email": {
    it: "Prepara una bozza email con AI senza inviarla.",
    en: "Prepares an AI email draft without sending it.",
  },
  "format-text-ai": {
    it: "Riformatta un contenuto seguendo un template documento con AI.",
    en: "Reformats content with AI following a document template.",
  },
  "format-text-template": {
    it: "Applica placeholder di un template documento senza usare AI.",
    en: "Applies document-template placeholders without AI.",
  },
  "verify-route": {
    it: "Controlla dati con regole deterministiche e segnala le violazioni.",
    en: "Checks data with deterministic rules and reports violations.",
  },
  "human-review": {
    it: "Crea una richiesta persistente di attenzione nel workspace.",
    en: "Creates a persistent workspace attention request.",
  },
  "request-decision": {
    it: "Mette la run in attesa finche una persona decide.",
    en: "Pauses the run until a person decides.",
  },
  output: {
    it: "Mostra il risultato ricevuto.",
    en: "Shows the received result.",
  },
};
...
export function localizedFlowNodeLabel(
  label: string,
  kind: FlowNodeType,
  language: UiLanguage,
): string {
  return label === NODE_KIND_LABELS[kind] || label === NODE_KIND_LABELS_EN[kind]
    ? flowNodeLabel(kind, language)
    : label;
}
...
export const TOOLBAR_GROUPS: Array<{
  ...
  title: string;
  titleEn: string;
  category: CanvasNodeData["paletteKind"];
  items: FlowNodeType[];
}> = [
  {
    id: "input",
  ...
  },
  {
    ...
    titleEn: "Agents",
    category: "AGENT",
    items: ["llm", "structure-data", "compose-email", "format-text-ai"],
  },
  {
    id: "tools",
    title: "Strumenti",
  ...
  },
  {
    ...
    titleEn: "Report",
    category: "REPORT",
    items: ["schedule", "send-email", "send-telegram", "send-whatsapp"],
  },
  {
    id: "brainyware",
    title: "Brainyware",
    titleEn: "Brainyware",
    category: "BRAINYWARE",
    items: ["brainyware-inference"],
  },
];

export const NODE_CATEGORY_BORDER: Record<
  CanvasNodeData["paletteKind"],
  string
> = {
...
export const NODE_CATEGORY_TAB_ACTIVE: Record<
  CanvasNodeData["paletteKind"],
  string
> = {
  ...
  AGENT:
    "border-status-progress-text bg-status-progress-bg text-status-progress-text",
...
export const NODE_CATEGORY_BADGE_TONE: Record<
  CanvasNodeData["paletteKind"],
  "info" | "success" | "warn" | "progress" | "danger"
> = {
  INPUT: "info",
  OUTPUT: "success",
  AGENT: "progress",
...
export const textareaClassName =
  "min-h-32 w-full rounded-[var(--radius-md)] border border-border-default bg-bg-muted p-3 text-sm text-text-secondary focus-visible:border-brand-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring-primary";
...
export function buildModuleCards(
  ...
  const visibleModuleKeys = enabledModuleKeys
    ? new Set(enabledModuleKeys)
    : null;
  const nonPlaygroundWorkflows = workflows.filter(
    (workflow) => workflow.key !== PLAYGROUND_KEY,
  );
  ...
  return moduleKeys.map((moduleKey) => {
    const workflow =
      nonPlaygroundWorkflows.find(
        (item) => item.moduleKey === moduleKey && item.isDefault,
      ) ??
    ...
    return {
      ...
      moduleKey,
      title:
        (language === "en" ? moduleLabelsEn : moduleLabels)[moduleKey] ??
        moduleKey.replace(/_/g, " "),
      description:
        (language === "en" ? moduleDescriptionsEn : moduleDescriptions)[
          moduleKey
        ] ??
        (language === "en"
          ? "Configurable modular workflow."
          : "Workflow modulare configurabile."),
      workflow,
      isPlayground: false,
    };
  });
...
export function isAgentTool(tool: WorkflowTool): boolean {
  return (
    tool.handlerKey === "langchain_orchestrator.structure_text" ||
    tool.handlerKey === "langchain_orchestrator.compose_email" ||
    tool.handlerKey === "langchain_orchestrator.format_text"
  );
}

export function isAdvancedLangChainTool(tool: WorkflowTool): boolean {
  return (
    tool.handlerKey === "langchain_orchestrator.chat" ||
    tool.handlerKey === "langchain_orchestrator.pipeline_execute"
  );
}
...
export function isAiRequestFlowNodeType(kind: FlowNodeType): boolean {
  return (
    ...
    kind === "compose-email" ||
    kind === "format-text-ai" ||
    kind === "document-set-ai" ||
    kind === "brainyware-inference"
  );
}

export function currentPromptFromConfiguration(
  configuration: Record<string, unknown>,
): string {
  return firstConfiguredString(
    ...
    configuration.extra_instructions,
  );
}

export function defaultPromptFromConfiguration(
  configuration: Record<string, unknown>,
): string {
  return firstConfiguredString(
    configuration.defaultPrompt,
    configuration.default_prompt,
    configuration.originalPrompt,
  );
}
...
export function normalizeAiPromptConfiguration(
  ...
  const currentPrompt =
    currentPromptFromConfiguration(configuration) || fallbackCurrentPrompt;
  const defaultPrompt =
    defaultPromptFromConfiguration(configuration) ||
    fallbackDefaultPrompt ||
    currentPrompt;
  return {
    ...configuration,
...
export function findToolForFlowNodeType(
  kind: FlowNodeType,
  tools: WorkflowTool[],
): WorkflowTool | null {
...
export function inferFlowNodeType(
  node: DraftNode,
  ...
  if (node.nodeKind === "INPUT") {
    return node.inputKind === "document" || node.inputKind === "file"
      ? "input-file"
      : "input-text";
  }
  ...
  if (
    tool?.handlerKey === "mail_engine.send_email" ||
    tool?.handlerKey === "mail_engine.send_quotation_email"
  ) {
    return "send-email";
  }
...
export function toolIcon(tool: WorkflowTool) {
  ...
  if (
    tool.handlerKey.includes("docx") ||
    tool.handlerKey.includes("document")
  ) {
...
export function toDraftEdge(
  item: WorkflowEdgeDto,
  idToClientId: Map<string, string>,
): DraftEdge {
  return {
    clientId: item.id,
    id: item.id,
...
export function toFlowNode(
  ...
  const tool = item.moduleToolId
    ? (tools.get(item.moduleToolId) ?? null)
    : null;
  const agent = item.moduleAgentId
    ? (agents.get(item.moduleAgentId) ?? null)
    : null;
  const flowType = inferFlowNodeType(item, tool);
  const isReportNode =
    flowType === "schedule" ||
    flowType === "send-email" ||
    flowType === "send-telegram" ||
    flowType === "send-whatsapp";
  const isBrainywareNode = tool?.handlerKey.startsWith("brainyware.") === true;
  const paletteKind = isReportNode
    ? "REPORT"
    : isBrainywareNode
      ? "BRAINYWARE"
      : item.nodeKind === "AGENT" || (tool && isLangChainTool(tool))
        ? "AGENT"
        : item.nodeKind === "TOOL"
          ? "TOOL"
          : item.nodeKind;
  ...
  return {
    ...
    data: {
      ...
      subtitle: agent
        ...
        : tool
          ? toolSubtitle(tool)
          : (item.inputKind ?? item.outputKind ?? item.nodeKind),
      paletteKind,
...
export function parsePaletteDragPayload(
  value: string,
...
export function formatResultPreview(value: unknown): string {
  ...
  const publishedOutputs = Array.isArray(record.published_outputs)
    ? record.published_outputs
    : [];
  const firstPublished =
    publishedOutputs[0] &&
    typeof publishedOutputs[0] === "object" &&
    publishedOutputs[0] !== null
      ? (publishedOutputs[0] as Record<string, unknown>)
      : null;
  if (firstPublished) {
    const publishedValue = firstPublished.value;
    const publishedText =
      typeof publishedValue === "string" ? publishedValue.trim() : "";
    if (publishedText) return publishedText;
    if (typeof firstPublished.label === "string" && firstPublished.label.trim())
      return firstPublished.label.trim();
  }
