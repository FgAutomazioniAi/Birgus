...
class GarageObjectPath:
    ...
    def parse(storage_path: str) -> "GarageObjectPath":
        ...
        return GarageObjectPath(
            bucket=without_scheme[:slash_index],
            key=without_scheme[slash_index + 1 :],
        )
...
class GarageStorageService:
    def __init__(self, settings: Settings):
        ...
        self._client: BaseClient = boto3.client(
            "s3",
            endpoint_url=self._settings.garage_s3_endpoint,
            ...
            config=boto3.session.Config(s3={"addressing_style": "path" if self._settings.garage_s3_force_path_style else "virtual"}),
        )
