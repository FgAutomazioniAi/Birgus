...
async function ask(question: string, defaultValue?: string): Promise<string> {
  ...
  try {
    ...
    return (
      (await readline.question(`${question}${suffix}: `)).trim() ||
      defaultValue ||
      ""
    );
  } finally {
...
async function askSecret(
  question: string,
  mayKeepExisting: boolean,
): Promise<string> {
  if (!stdin.isTTY || typeof stdin.setRawMode !== "function") {
    throw new Error("Il comando richiede un terminale interattivo.");
  }
  stdout.write(
    `${question}${mayKeepExisting ? " (Invio per conservare quella esistente)" : ""}: `,
  );
    ...
    const onData = (chunk: string): void => {
      for (const character of chunk) {
        if (character === "\u0003") {
          finish();
          rejectPromise(
            new Error("Configurazione annullata: nessuna modifica applicata."),
          );
          return;
        }
  ...
  await new Promise<void>((resolvePromise, rejectPromise) => {
    ...
    child.once("exit", (code) =>
      code === 0
        ? resolvePromise()
        : rejectPromise(
            new Error(`${command} terminato con codice ${code ?? -1}.`),
          ),
    );
  });
}
...
async function main(): Promise<void> {
  if (!existsSync(ENV_PATH))
    throw new Error("File .env non trovato: eseguire prima npm run setup.");
  const envContent = readFileSync(ENV_PATH, "utf8");
  const currentBaseUrl =
    parseEnv(envContent).get("BRAINYWARE_BASE_URL")?.trim() || undefined;
  const baseUrl = await ask(
    "URL base Brainyware (es. https://brainy.example.com)",
    currentBaseUrl,
  );
  ...
  if (!accessKey && !accessKeyExists)
    throw new Error("L'Access Key e' obbligatoria.");
  if (!secretKey && !secretKeyExists)
    throw new Error("La Secret Key e' obbligatoria.");

  if (accessKey) atomicWrite(ACCESS_KEY_PATH, accessKey);
  if (secretKey) atomicWrite(SECRET_KEY_PATH, secretKey);
  const updatedEnv = setEnvValues(
    envContent,
    new Map([
