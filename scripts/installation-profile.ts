...
function configuredWorkflowToolKeys(): string[] | null {
  const values = [
    ...new Set(
      (process.env.WORKFLOW_STANDARD_TOOL_KEYS ?? "")
        .split(",")
...
export function createInstallationProfile(
  workspaces: InstallationProfile["workspaces"],
): InstallationProfile {
  return {
    ...
    workspaces: workspaces
      ...
      .sort((left, right) =>
        left.workspace_code.localeCompare(right.workspace_code),
      ),
