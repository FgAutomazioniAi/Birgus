...
function readArguments(): Arguments | null {
  ...
  if (!/^[a-z0-9][a-z0-9-]{1,62}$/.test(code)) {
    throw new Error(
      "Workspace code must contain only lowercase letters, numbers, and hyphens.",
    );
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!administrator) {
      throw new Error(
        `Active user '${args.adminEmail}' not found. Create the user first.`,
      );
    }
    if (!adminRole) {
      throw new Error(
        "System role 'admin' not found. Run the normal database seed first.",
      );
    }
    ...
    if (!template) {
      throw new Error(
        `Template workspace '${args.templateCode}' not found in organization '${organization.code}'.`,
      );
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (existing) {
        throw new Error(
          `Workspace '${args.code}' already exists (${existing.id}). No changes were made.`,
        );
      }
      ...
      await tx.workspaceModule.createMany({
        data: template.workspace_modules.map((item) => ({
          workspace_id: workspace.id,
          module_id: item.module_id,
          is_enabled: item.is_enabled,
          configured_by_user_id: administrator.id,
        })),
      });
      await tx.projectStatus.createMany({
        data: template.project_statuses.map((item) => ({
          workspace_id: workspace.id,
          key: item.key,
          label: item.label,
        })),
      ...
      await tx.projectRevision.createMany({
        data: template.project_revisions.map((item) => ({
          workspace_id: workspace.id,
          code: item.code,
        })),
      });
      ...
      for (const tool of template.module_tools) {
        const created = await tx.moduleTool.create({
          data: {
            ...
            input_schema: copyJson(tool.input_schema),
            output_schema: copyJson(tool.output_schema),
            configuration: copyJson(tool.configuration),
      ...
      for (const workflow of template.module_workflows) {
        const createdWorkflow = await tx.moduleWorkflow.create({
          data: {
            ...
            configuration: copyJson(workflow.configuration),
        ...
        for (const node of workflow.nodes) {
          const createdNode = await tx.moduleWorkflowNode.create({
            data: {
              ...
              module_agent_id: node.module_agent_id
                ? (agentIdMap.get(node.module_agent_id) ?? null)
                : null,
              module_tool_id: node.module_tool_id
                ? (toolIdMap.get(node.module_tool_id) ?? null)
                : null,
              input_kind: node.input_kind,
              output_kind: node.output_kind,
              configuration: copyJson(node.configuration),
              input_schema: copyJson(node.input_schema),
              output_schema: copyJson(node.output_schema),
        ...
        await tx.moduleWorkflowEdge.createMany({
          data: workflow.edges.map((edge) => ({
            ...
            label: edge.label,
            condition_payload: copyJson(edge.condition_payload),
      ...
      return {
        id: workspace.id,
        modules: template.workspace_modules.length,
        workflows: template.module_workflows.length,
      };
    });
    ...
    console.log(
      JSON.stringify(
        {
          workspace: { id: result.id, code: args.code, name: args.name },
          administrator: administrator.email,
          template: args.templateCode,
