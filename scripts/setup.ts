...
function isPlaceholder(value: string | undefined): boolean {
  return (
    !value || value.includes("CHANGE_ME") || value.includes("REPLACE_WITH")
  );
}
...
function setTomlValue(content: string, key: string, value: string): string {
  ...
  if (!pattern.test(content))
    throw new Error(`Garage template is missing '${key}'.`);
...
export function nextGarageLayoutVersion(output: string): string | null {
  const instructedVersion = output.match(
    /layout apply(?:\s+|\s+.*\s+)--version(?:=|\s+)(\d+)/i,
  )?.[1];
  if (instructedVersion) return instructedVersion;
  const stagedVersion = output.match(
    /staged(?: cluster)? layout version\s*:\s*(\d+)/i,
  )?.[1];
  if (stagedVersion) return stagedVersion;
  const currentVersion = output.match(
    /current(?: cluster)? layout version\s*:\s*(\d+)/i,
  )?.[1];
...
async function askCode(question: string, suggested: string): Promise<string> {
  while (true) {
    ...
    stdout.write(
      "Usare da 2 a 63 caratteri: lettere minuscole, numeri e trattini.\n",
    );
  }
}
...
async function askCapacity(defaultValue: string): Promise<string> {
  while (true) {
    const value = (
      await askRequired("Capacita' assegnata a Garage", defaultValue)
    ).toUpperCase();
...
async function collectAnswers(
  ...
  const organizationCode = await askCode(
    "Codice organizzazione",
    slug(organizationName) || "azienda",
  );
  const workspaceName = await askRequired(
    "Nome del workspace",
    organizationName,
  );
  const workspaceCode = await askCode(
    "Codice workspace",
    slug(workspaceName) || "principale",
  );
  ...
  const aiModel = await askRequired(
    "Nome del modello AI",
    existing.get("AI_PROVIDER_CHAT_MODEL") || "birgus-vl",
  );
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    child.on("close", (code) => {
      ...
      if (exitCode !== 0 && !options.allowFailure) {
        rejectPromise(
          new Error(
            output.trim() || `${command} exited with code ${exitCode}.`,
          ),
        );
        return;
      }
...
function prepareConfiguration(
  ...
  const values = new Map<string, string>([
    ...
    [
      ...
      secret("AUTH_TOTP_ENCRYPTION_KEY", () => generatedHex()),
    ],
    ["GARAGE_RPC_SECRET", secret("GARAGE_RPC_SECRET", () => generatedHex())],
    [
      "GARAGE_ADMIN_TOKEN",
      secret("GARAGE_ADMIN_TOKEN", () => generatedToken()),
    ],
    [
      "GARAGE_METRICS_TOKEN",
      secret("GARAGE_METRICS_TOKEN", () => generatedToken()),
    ],
    [
      "GARAGE_S3_ACCESS_KEY_ID",
      secret("GARAGE_S3_ACCESS_KEY_ID", () => `GK${generatedHex(12)}`),
    ],
    [
      "GARAGE_S3_SECRET_ACCESS_KEY",
      secret("GARAGE_S3_SECRET_ACCESS_KEY", () => generatedHex()),
    ],
    [
      "OCR_LIFECYCLE_TOKEN",
      secret("OCR_LIFECYCLE_TOKEN", () => generatedToken()),
    ],
    ...
    [
      "AI_PROVIDER_API_KEY",
      answers.aiApiKey || existing.get("AI_PROVIDER_API_KEY") || "",
    ],
  ...
  garageContent = setTomlValue(
    garageContent,
    "rpc_secret",
    values.get("GARAGE_RPC_SECRET")!,
  );
  garageContent = setTomlValue(
    garageContent,
    "admin_token",
    values.get("GARAGE_ADMIN_TOKEN")!,
  );
  garageContent = setTomlValue(
    garageContent,
    "metrics_token",
    values.get("GARAGE_METRICS_TOKEN")!,
  );
...
function backupExistingConfiguration(): string | null {
  ...
  if (existsSync(ENV_PATH))
    copyFileSync(ENV_PATH, join(backupDirectory, ".env"));
  if (existsSync(GARAGE_CONFIG_PATH))
    copyFileSync(
      GARAGE_CONFIG_PATH,
      join(backupDirectory, "garage.local.toml"),
    );
  return backupDirectory;
}

async function configureGarage(
  values: Map<string, string>,
  capacity: string,
): Promise<void> {
  const status = await dockerCompose(
    ["exec", "-T", "garage", "/garage", "status"],
    { capture: true },
  );
  const nodeId = status.output.match(/^([0-9a-f]{16})\s+/m)?.[1];
  if (!nodeId)
    throw new Error("Impossibile rilevare l'identificativo del nodo Garage.");

  const nodeStatusLine =
    status.output.match(new RegExp(`^${nodeId}\\s+.*$`, "m"))?.[0] ?? "";
  const nodeHasRole =
    !nodeStatusLine.includes("NO ROLE ASSIGNED") &&
  ...
  if (!nodeHasRole) {
    ...
    if (!version)
      throw new Error(
        `Garage non ha restituito una versione di layout riconoscibile.\n${stagedLayout.output}`,
      );
    await dockerCompose([
      "exec",
      "-T",
  ...
  const keyInfo = await dockerCompose(
    ["exec", "-T", "garage", "/garage", "key", "info", keyId],
    {
      capture: true,
      allowFailure: true,
    },
  ...
  const bucketInfo = await dockerCompose(
    ["exec", "-T", "garage", "/garage", "bucket", "info", bucket],
    {
      capture: true,
      allowFailure: true,
    },
...
async function main(): Promise<void> {
  ...
  if (!stdin.isTTY || !stdout.isTTY)
    throw new Error("Eseguire npm run setup da un terminale interattivo.");

  heading("Installazione guidata Birgus");
  stdout.write(
    "Il programma prepara la configurazione, avvia Docker e crea il primo account Developer.\n",
  );

  const existingContent = existsSync(ENV_PATH)
  ...
  if (existingContent || existsSync(GARAGE_CONFIG_PATH)) {
    ...
    const resume = await askYesNo(
      "Creare un backup e continuare usando i segreti esistenti",
      false,
    );
    if (!resume) {
  ...
  stdout.write(
    `Workspace: ${answers.workspaceName} (${answers.workspaceCode})\n`,
  );
  stdout.write(
    `Developer: ${answers.firstName}${answers.lastName ? ` ${answers.lastName}` : ""} <${answers.email}>\n`,
  );
  ...
  for (const [key, value] of parseEnv(configuration.envContent))
    process.env[key] = value;
  stdout.write(
    "Configurazione salvata. I segreti non vengono mostrati a schermo.\n",
  );
  ...
  stdout.write(
    `Accedere con ${answers.email}. Al primo accesso saranno richiesti cambio password e configurazione 2FA.\n`,
  );
}
...
if (
  ...
  main().catch((error: unknown) => {
    ...
    console.error(
      "Le configurazioni gia' scritte sono state conservate. Correggere il problema e rieseguire npm run setup.",
    );
    process.exitCode = 1;
  });
}
