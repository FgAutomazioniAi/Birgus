...
const tools = [
  {
    ...
    description:
      "Interroga un database Brainyware o un modello senza creare una sessione persistente.",
  ...
  },
  {
    ...
    label: "AI Chat",
    description:
      "Esegue una richiesta chat generica tramite orchestratore Python LangChain.",
  ...
  },
  {
    ...
    description:
      "Prepara oggetto e corpo email senza invio, con tono professionale, formale o informale.",
    runtimeKind: WorkflowToolRuntimeKind.PYTHON_MODULE,
  ...
  },
  {
    ...
    description:
      "Applica un template documento al contenuto usando il motore AI.",
    runtimeKind: WorkflowToolRuntimeKind.PYTHON_MODULE,
  ...
  },
  {
    ...
    description:
      "Formatta testo in modo deterministico sostituendo i placeholder del template documento.",
    runtimeKind: WorkflowToolRuntimeKind.BACKEND,
  ...
  },
  {
    ...
    description:
      "Valuta regole deterministiche sui dati del workflow senza usare AI.",
    runtimeKind: WorkflowToolRuntimeKind.BACKEND,
  ...
  },
  {
    ...
    description:
      "Sospende il workflow e lo riprende solo dopo una decisione umana esplicita.",
    runtimeKind: WorkflowToolRuntimeKind.BACKEND,
...
async function main(): Promise<void> {
  ...
  const unknownKeys = [...selectedKeys].filter(
    (key) => !tools.some((tool) => tool.key === key),
  );
  if (unknownKeys.length > 0)
    throw new Error(
      `Unknown WORKFLOW_STANDARD_TOOL_KEYS: ${unknownKeys.join(", ")}`,
    );
  ...
  if (!module)
    throw new Error(
      "Modulo workflow_management non trovato. Eseguire il bootstrap iniziale una sola volta.",
    );
  const workspaces = await prisma.workspaceModule.findMany({
    ...
    for (const tool of tools) {
      ...
      await prisma.moduleTool.upsert({
        ...
        update: {
          ...
          runtime_kind: tool.runtimeKind,
          handler_key: tool.handlerKey,
  ...
  console.log(
    `Workflow tools synchronized for ${workspaces.length} workspace(s).`,
  );
}
