...
const backendUrl =
  process.env.BIRGUS_BACKEND_URL?.trim() || "http://localhost:3000";
const frontendUrl =
  process.env.BIRGUS_FRONTEND_URL?.trim() || "http://localhost:3100";
...
async function main(): Promise<void> {
  ...
  assert(
    ...
    "Workflow endpoint must reject unauthenticated access.",
  );
  assert(
    connectedAppsResponse.status === 401,
    "Connected apps endpoint must reject unauthenticated access.",
  );

  const databaseUrl =
    process.env.BIRGUS_DATABASE_URL?.trim() ||
    process.env.DATABASE_URL?.trim() ||
    readDatabaseUrlFromEnv();
  if (!databaseUrl) {
    throw new Error(
      "DATABASE_URL non disponibile per la verifica dei workflow. Impostare BIRGUS_DATABASE_URL per eseguire il test dall'host.",
    );
  }
  ...
  try {
    ...
    const summaries = workflows.map((workflow) => {
      assert(
        ...
        `Workflow '${workflow.key}' senza nodi.`,
      );
      assertNoDuplicateNodeKeys(
        workflow.key,
        workflow.nodes.map((node) => node.node_key),
      );
      ...
      for (const node of workflow.nodes) {
        if (node.node_kind === "TOOL") {
          assert(
            node.module_tool?.is_enabled && node.module_tool.handler_key.trim(),
            `Tool non configurato in '${workflow.key}:${node.node_key}'.`,
          );
        }
        if (node.node_kind === "AGENT") {
          assert(
            node.module_agent?.is_enabled,
            `Agent non configurato in '${workflow.key}:${node.node_key}'.`,
          );
        }
      }

      const order = planner.buildExecutionOrder(
        workflow.nodes,
        workflow.edges,
        () => true,
      );
      assert(
        order.length > 0,
        `Workflow '${workflow.key}' senza piano di esecuzione.`,
      );
...
function assertNoDuplicateNodeKeys(workflowKey: string, keys: string[]): void {
  assert(
    ...
    `Workflow '${workflowKey}' contiene node_key duplicati.`,
  );
}
...
function assertNoDanglingEdges(
  ...
  for (const edge of edges) {
    assert(
      validIds.has(edge.source_node_id) && validIds.has(edge.target_node_id),
      `Workflow '${workflowKey}' contiene un collegamento non valido.`,
    );
  }
}
...
function assertNoCycle(
  ...
  const activeIds = new Set(
    nodes
      .filter((node) => node.is_enabled || node.is_required)
      .map((node) => node.id),
  );
  ...
  for (const edge of edges) {
    if (
      ...
      activeIds.has(edge.target_node_id)
    ) {
  ...
  const visit = (nodeId: string): void => {
    assert(
      !visiting.has(nodeId),
      `Workflow '${workflowKey}' contiene un ciclo.`,
    );
...
function assertWorkflowHandleContracts(
  ...
  for (const edge of edges) {
    ...
    const isVerifySource =
      source?.module_tool?.handler_key === "workflow_logic.verify_and_route";
    const isVerifyTarget =
      target?.module_tool?.handler_key === "workflow_logic.verify_and_route";

    if (edge.source_handle === "valid" || edge.source_handle === "invalid") {
      assert(
        isVerifySource,
        `Workflow '${workflowKey}' usa l'uscita ${edge.source_handle} da un nodo che non e Verifica e instrada.`,
      );
    }
    if (isVerifySource) {
      assert(
    ...
    assert(
      isVerifyTarget,
      `Workflow '${workflowKey}' collega una regola a un nodo che non e Verifica e instrada.`,
    );
    ...
    assert(
      ...
      `Workflow '${workflowKey}' collega una regola non piu presente nel nodo Verifica e instrada.`,
    );
  }
}
