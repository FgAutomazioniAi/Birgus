...
export class TenancyGuard {
  ...
  public async ensureWorkspaceAccess(
    workspaceId: string,
    userId: string,
  ): Promise<void> {
    const hasAccess = await this.membershipReader.isUserActiveInWorkspace(
      workspaceId,
      userId,
    );
    ...
    if (!hasAccess) {
      throw new AppError(
        "User is not active in workspace.",
  ...
  public async resolveWorkspaceIdForUser(
    userId: string,
    ...
    const workspaceId =
      await this.membershipReader.findPrimaryWorkspaceIdForUser(userId);
    if (!workspaceId) {
      throw new AppError(
        "No active workspace available for user.",
