...
export class LocalLmOrchestrator {
  ...
  public async analyzeQuotationFromStorage(input: {
    ...
    const inferenceStartedAt = Date.now();
    const lmResponse = await this.lmClient.completeJsonSchema({
      systemPrompt,
  ...
  private async requestDdtAnalysis(input: {
    ...
    this.logLmEvent("LM_REQUEST", {
      userContextChars: input.userContext.length,
      systemPromptChars: input.systemPrompt.length,
      jsonSchema: this.useJsonSchema,
    });
    ...
    if (this.useJsonSchema) {
      ...
      const result = {
        ...
        rawProvider: "openai-compatible-chat-completions-json-schema",
      };
      this.logLmEvent("LM_RESPONSE", {
    ...
    const result = {
      ...
      rawProvider: "openai-compatible-chat",
    };
    this.logLmEvent("LM_RESPONSE", {
      provider: result.rawProvider,
  ...
  private logLmEvent(event: string, meta: Record<string, unknown>): void {
    ...
    console.log(`[Birgus][LocalLmOrchestrator][${event}]`, meta);
  }
  ...
  private async executePythonModule(
    ...
    return {
      ok: payload.ok === true,
      module: typeof payload.module === "string" ? payload.module : module,
