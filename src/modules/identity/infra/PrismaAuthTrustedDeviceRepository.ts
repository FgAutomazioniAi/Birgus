...
export class PrismaAuthTrustedDeviceRepository
  implements AuthTrustedDeviceRepository
{
  ...
  public async touch(id: string, now: Date): Promise<void> {
    const prisma = PrismaClientManager.getClient();
    await prisma.authTrustedDevice.update({
      where: { id },
  ...
  public async revokeAllForUser(userId: string, now: Date): Promise<void> {
    const prisma = PrismaClientManager.getClient();
    await prisma.authTrustedDevice.updateMany({
