...
export class AiProviderSettingsService {
  ...
  private async buildOneShotClient(
    patch: AiProviderSettingsPatch,
  ): Promise<OpenAiCompatibleLmClient> {
  ...
  private normalizePatch(
    ...
    return this.compactSettings({
      ...
      timeoutMs:
        ...
        patch.timeoutMs > 0
          ? Math.trunc(patch.timeoutMs)
          : undefined,
      maxOutputTokens: this.toBoundedInteger(
      ...
      seed:
        patch.seed === null
          ? null
          : this.toBoundedInteger(patch.seed, 0, 2_147_483_647),
      contextTokenLimit:
        patch.contextTokenLimit === null
          ? null
          : this.toBoundedInteger(patch.contextTokenLimit, 256, 8192),
    });
  }
    ...
    return this.compactSettings({
      provider:
        typeof row.provider === "string"
          ? normalizeAiProviderId(row.provider)
          : undefined,
      ...
      timeoutMs:
        ...
        row.timeoutMs > 0
          ? Math.trunc(row.timeoutMs)
          : undefined,
      maxOutputTokens: this.clampInteger(
        row.maxOutputTokens,
        1,
        MAX_AI_PROVIDER_OUTPUT_TOKENS,
      ),
      ...
      seed:
        row.seed === null
          ? null
          : this.toBoundedInteger(row.seed, 0, 2_147_483_647),
      contextTokenLimit:
        row.contextTokenLimit === null
          ? null
          : this.toBoundedInteger(row.contextTokenLimit, 256, 8192),
    });
  }
