...
export class CommissionVendorListService {
  public async get(workspaceId: string): Promise<VendorListConfiguration> {
    ...
    return {
      categories: await this.loadDefaultCategories(),
      revision: 0,
      isCustomized: false,
    };
  }
  ...
  public async save(
    workspaceId: string,
    categories: VendorListCategory[],
  ): Promise<VendorListConfiguration> {
    const normalized = this.normalizeCategories(categories);
    if (!normalized.length) {
      throw new AppError(
        "La Vendor List deve contenere almeno una categoria.",
        "VENDOR_LIST_EMPTY",
        400,
      );
    }
    ...
    const revision =
      (this.parseMetadata(existing?.metadata)?.revision ?? 0) + 1;
    const metadata = { revision, categories: normalized };

    if (existing) {
      await prisma.commissionCatalog.update({
      ...
      await prisma.commissionCatalog.create({
        data: {
          ...
          description:
            "Configurazione personalizzata della Vendor List per le nuove checklist.",
          metadata: metadata as unknown as Prisma.InputJsonValue,
        },
      });
    }
  ...
  private async loadDefaultCategories(): Promise<VendorListCategory[]> {
    ...
    const table = await prisma.commissionTableDefinition.findFirst({
      where: {
        ...
        field: { version: { workspace_id: null, status: "PUBLISHED" } },
      },
  ...
  private parseMetadata(
  ...
  ): Omit<VendorListConfiguration, "isCustomized"> | null {
    ...
    return {
      ...
      categories: this.normalizeCategories(
        value.categories as VendorListCategory[],
      ),
    };
  }

  private extractDefaultRows(
    metadata: Prisma.JsonValue | null | undefined,
  ...
  private rowsToCategories(
    ...
    for (const row of rows) {
      ...
      category.items.push({
        component: this.clean(row.col_2_componente),
        brands:
          this.clean(row.col_3_marche_di_riferimento) ||
          this.clean(row._placeholder_col_3_marche_di_riferimento),
      });
    ...
    return categories
      .map((category) => ({
        ...
        items: Array.isArray(category?.items)
          ? category.items
              .map((item) => ({
                component: this.clean(item?.component),
                brands: this.clean(item?.brands),
              }))
