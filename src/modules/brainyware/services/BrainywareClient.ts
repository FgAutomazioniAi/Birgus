...
export class BrainywareClient {
  ...
  public constructor(options: BrainywareClientOptions = {}) {
    this.baseUrl = this.normalizeBaseUrl(
      options.baseUrl ?? process.env.BRAINYWARE_BASE_URL ?? "",
    );
  ...
  public async listModels(): Promise<BrainywareModel[]> {
    ...
    const explicitLanguageModels = models.filter((model) =>
      /^llm[-_:]/i.test(model.id),
    );
    if (explicitLanguageModels.length > 0) return explicitLanguageModels;
    return models.filter(
      (model) =>
        !/(^|[-_:])(stt|tts|embedding|rerank)([-_:]|$)/i.test(model.id),
    );
  }

  public async listDatabaseConnections(): Promise<
    BrainywareDatabaseConnection[]
  > {
    const payload = await this.request(
      "/brainy/api/database/connections/lf/list",
    );
    const record = this.toRecord(payload);
      ...
      return [
        {
          id,
          name:
            this.firstString(row.name, row.label, row.connection_name) ||
            `Connessione ${id}`,
  ...
  public async inferStateless(params: {
    ...
    if (message.length > 100_000)
      throw new Error(
        "Il testo per l'inferenza Brainyware supera 100000 caratteri.",
      );
    const instructions = (params.instructions ?? "").trim();
    if (instructions.length > 4_000)
      throw new Error("Le istruzioni Brainyware superano 4000 caratteri.");
  ...
  public async assertReadOnlyDatabaseConnection(
    connectionId: string,
  ): Promise<void> {
    const connection = (await this.listDatabaseConnections()).find(
      (item) => item.id === connectionId,
    );
    if (!connection)
      throw new Error(
        "La connessione database Brainyware non e' accessibile al service account.",
      );
    if (connection.accessMode?.toUpperCase() !== "READ_ONLY") {
      throw new Error(
        "Birgus consente esclusivamente connessioni database Brainyware READ_ONLY.",
      );
    }
  }
  ...
  public async createAgentSession(
    agentId: string,
    sessionName: string,
  ): Promise<BrainywareAgentSession> {
    const payload = await this.request(
      `/brainy/api/ai/agents/${encodeURIComponent(agentId)}/sessions`,
      {
        method: "POST",
    ...
    if (!id)
      throw new Error(
        "Brainyware non ha restituito l'identificativo della sessione.",
      );
    return { id };
  }
  ...
  public async runAgent(params: {
    ...
    try {
      const response = await this.fetchImpl(
        `${this.baseUrl}/brainy/api/ai/agents/${encodeURIComponent(params.agentId)}/run`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            ...(params.sessionId
              ? {}
              : { session_name: params.sessionName?.trim() || "Nuova chat" }),
            message,
          }),
      ...
      if (!result.reply)
        throw new Error("Brainyware ha risposto senza contenuto testuale.");
      return result;
    } catch (error) {
      if (error instanceof Error && error.name === "AbortError")
        throw new Error("Timeout durante l'esecuzione dell'agente Brainyware.");
      throw error;
    } finally {
  ...
  public async runAgentSingleShot(params: {
    agentId: string;
    message: string;
    sessionName?: string;
  }): Promise<BrainywareInferenceResult> {
    const result = await this.runAgent({
      ...
      sessionId: "",
      sessionName: params.sessionName ?? "Workflow Birgus",
      message: params.message,
    });
    if (result.sessionId)
      await this.deleteAgentSession(params.agentId, result.sessionId);
    return {
      reply: result.reply,
      mode: "agent",
  ...
  public async *runAgentStream(params: {
    agentId: string;
    sessionId: string;
    sessionName?: string;
    message: string;
    ...
    try {
      ...
      if (!response.body)
        throw new Error("Brainyware non ha aperto uno stream di risposta.");
  ...
  public async *runDatabaseStream(params: {
    ...
    try {
      const response = await this.fetchImpl(
        `${this.baseUrl}/brainy/api/database/connections/lf/${encodeURIComponent(params.connectionId)}/chat`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          body: JSON.stringify({
            ...
            locale: "italiano",
            instructions:
              "Rispondi in italiano. Interroga esclusivamente il database configurato.",
            stream: true,
          }),
          signal: controller.signal,
        },
      ...
      if (!response.ok)
        throw new BrainywareHttpError(
          response.status,
          `Interrogazione database Brainyware non riuscita (HTTP ${response.status}).`,
        );
      if (!response.body)
        throw new Error("Brainyware non ha aperto uno stream di risposta.");
      ...
      if (error instanceof Error && error.name === "AbortError")
        throw new Error(
          "Timeout durante l'interrogazione del database Brainyware.",
        );
      throw error;
    } finally {
      clearTimeout(timeout);
  ...
  public async searchKnowledge(query: string): Promise<string> {
    const token = await this.authenticate();
    const response = await this.fetchImpl(
      `${this.baseUrl}/brainy/api/ai/search`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (!response.ok)
      throw new BrainywareHttpError(
        response.status,
        `Ricerca knowledge Brainyware non riuscita (HTTP ${response.status}).`,
      );
  ...
  public async deleteAgentSession(
    ...
    await this.request(
      `/brainy/api/ai/agents/${encodeURIComponent(agentId)}/sessions/${encodeURIComponent(sessionId)}`,
      {
        method: "DELETE",
      },
    );
  ...
  private async queryDatabaseStateless(params: {
    ...
    if (!connectionId) {
      ...
      if (connections.length === 0)
        throw new Error(
          "Nessun database Brainyware accessibile al service account.",
        );
      if (connections.length > 1)
        throw new Error("Selezionare il database Brainyware da interrogare.");
      connectionId = connections[0].id;
    }
    await this.assertReadOnlyDatabaseConnection(connectionId);
    const payload = await this.request(
      `/brainy/api/database/connections/lf/${encodeURIComponent(connectionId)}/chat`,
      {
        method: "POST",
        body: {
          message: params.message,
  ...
  private async completeStateless(params: {
    ...
    if (!model) {
      ...
      if (models.length === 0)
        throw new Error(
          "Nessun modello Brainyware accessibile al service account.",
        );
      model = models[0].id;
    }
    const messages = [
    ...
    const payload = await this.request(
      "/brainy/api/openai/v1/chat/completions",
      {
        method: "POST",
        body: {
          model,
          ...
          max_tokens: Math.trunc(
            this.clampNumber(params.maxTokens, 1, 32_000, 800),
          ),
        },
      },
    );
  ...
  private async request(
    ...
    try {
      ...
      if (error instanceof Error && error.name === "AbortError")
        throw new Error("Timeout durante la richiesta a Brainyware.");
      throw error;
    } finally {
      clearTimeout(timeout);
    }
  ...
  private async authenticate(): Promise<string> {
    ...
    try {
      const response = await this.fetchImpl(
        `${this.baseUrl}/brainy/api/auth/service`,
        {
          method: "POST",
          headers: {
            Accept: "application/json",
      ...
      const token = this.firstString(
        payload.token,
        payload.access_token,
        this.toRecord(payload.data).token,
        this.toRecord(payload.data).access_token,
      );
      if (!token)
        throw new Error("Brainyware non ha restituito un token di accesso.");
      this.token = token;
      this.tokenExpiresAt = Date.now() + 7 * 60 * 60 * 1_000;
      return token;
    } catch (error) {
      if (error instanceof Error && error.name === "AbortError")
        throw new Error("Timeout durante l'autenticazione Brainyware.");
      throw error;
    } finally {
      clearTimeout(timeout);
    }
  ...
  private async readCredentials(): Promise<{
    ...
    const accessKey =
      this.configuredAccessKey ??
      (await this.readSecretFile("BRAINYWARE_ACCESS_KEY_FILE"));
    const secretKey =
      this.configuredSecretKey ??
      (await this.readSecretFile("BRAINYWARE_SECRET_KEY_FILE"));
    if (!accessKey || !secretKey)
      throw new Error("Credenziali Brainyware non configurate.");
    ...
    for (const line of source.split(/\r?\n/)) {
      ...
      try {
        const record = this.toRecord(
          JSON.parse(line.slice(5).trim()) as unknown,
        );
        sessionId ??=
        ...
        const role = this.firstString(
          record.role,
          this.toRecord(record.message).role,
        ).toLowerCase();
  ...
  private extractAgentStreamEvent(
    source: string,
    receivedChunk: boolean,
  ): { content: string; isChunk: boolean } | null {
    const event =
      source
        .match(/^event:\s*(.+)$/m)?.[1]
        ?.trim()
    ...
    try {
      ...
      if (!receivedChunk && (event === "END" || event === "RESULT"))
        return { content, isChunk: false };
    } catch {
      // Ignore malformed SSE fragments and preserve valid assistant chunks.
    }
    return null;
  ...
  private extractGenericStreamEvent(
    source: string,
    receivedChunk: boolean,
  ): { content: string; isChunk: boolean } | null {
    const event =
    ...
    if (event === "CHUNK" || event === "DELTA" || !event)
      return { content, isChunk: true };
    if (
      !receivedChunk &&
      (event === "END" || event === "RESULT" || event === "DONE")
    )
      return { content, isChunk: false };
    return null;
  }
  ...
  private extractKnowledgeText(value: unknown, depth = 0): string {
    ...
    if (Array.isArray(value))
      return value
        .map((item) => this.extractKnowledgeText(item, depth + 1))
        .filter(Boolean)
        .join("\n");
    const record = this.toRecord(value);
    const direct = this.firstString(
  ...
  private normalizeBaseUrl(value: string): string {
    ...
    if (!["http:", "https:"].includes(parsed.protocol))
      throw new Error("BRAINYWARE_BASE_URL deve usare HTTP o HTTPS.");
