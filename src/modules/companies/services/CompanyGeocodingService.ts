...
export class CompanyGeocodingService {
  ...
  public async geocode(
    ...
    if (!params.address.trim() || !params.city.trim()) {
      throw new AppError(
        "Indirizzo e citta sono necessari per cercare la posizione.",
  ...
  private async requestPosition(
    ...
    for (const url of new Map(
      attempts.map((attempt) => [attempt.toString(), attempt]),
    ).values()) {
  ...
  private buildStructuredUrl(params: CompanyAddress): URL {
    ...
    if (params.postalCode.trim())
      url.searchParams.set("postalcode", params.postalCode.trim());
  ...
  private async requestNominatim(
    url: URL,
  ): Promise<CompanyGeocodingResult | null> {
    const waitMs = Math.max(
      0,
      this.lastRequestAt + this.minimumIntervalMs - Date.now(),
    );
    if (waitMs > 0) {
    ...
    try {
      ...
      throw new AppError(
        "Il servizio di geocodifica non e raggiungibile.",
    ...
    try {
      ...
      throw new AppError(
        "Il servizio di geocodifica ha restituito una risposta non valida.",
    ...
    const latitude =
      typeof first?.lat === "string" ? Number(first.lat) : Number.NaN;
    const longitude =
      typeof first?.lon === "string" ? Number(first.lon) : Number.NaN;
    if (
      ...
      throw new AppError(
        "Il servizio di geocodifica ha restituito coordinate non valide.",
        "GEOCODING_RESPONSE_INVALID",
        502,
      );
    }
    ...
    return {
      ...
      displayName:
        typeof first?.display_name === "string"
          ? first.display_name
          : (url.searchParams.get("q") ?? paramsToDisplayName(url)),
      provider: "OpenStreetMap Nominatim",
    };
  }
}
