...
export class WorkflowService {
  ...
  public constructor(
    ...
    runDispatcher?: WorkflowRunDispatcher | null,
  ...
  public async findWorkflowByKey(
    ...
    return this.repository.findWorkflowByKey(
      workspaceId,
      moduleKey,
      workflowKey,
    );
  ...
  public async saveWorkflowDefinition(
    input: Parameters<WorkflowRepository["saveWorkflowDefinition"]>[0],
  ): Promise<ModuleWorkflowEntity> {
    if (!input.key.trim()) {
      throw new AppError(
    ...
    if (input.workflowId && input.actorUserId) {
      await this.lockService.heartbeat(
        input.workspaceId,
        input.workflowId,
        input.actorUserId,
      );
  ...
  public async createWorkflowRun(
    input: Parameters<WorkflowRepository["createWorkflowRun"]>[0],
  ): Promise<ModuleWorkflowRunEntity> {
  ...
  public async deletePersonalWorkflow(
    ...
    if (workflow.moduleKey !== "workflow_management") {
      throw new AppError(
        "Solo i workflow del Playground possono essere eliminati.",
        "WORKFLOW_DELETE_NOT_ALLOWED",
        403,
      );
    }
    await this.repository.deletePersonalWorkflow(
      workspaceId,
      workflowId,
      actorUserId,
    );
