...
export class HumanInterventionService {
  ...
  public async countOpenForUser(
    ...
    return prisma.humanIntervention.count({
      where: {
        ...
        status: {
          in: [HumanInterventionStatus.OPEN, HumanInterventionStatus.IN_REVIEW],
        },
  ...
  public async decide(params: {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (
        item.status === HumanInterventionStatus.RESOLVED ||
        item.status === HumanInterventionStatus.CANCELED
      ) {
        throw new AppError(
          "Intervento gia chiuso.",
          "HUMAN_INTERVENTION_ALREADY_CLOSED",
          409,
        );
      }
      if (item.assigned_user_id && item.assigned_user_id !== params.userId) {
        throw new AppError(
          "L'intervento e assegnato a un altro utente.",
          "HUMAN_INTERVENTION_NOT_ASSIGNEE",
          403,
        );
      }
      ...
      await tx.moduleWorkflowRunStep.updateMany({
        where: {
          ...
          status: WorkflowStepStatus.WAITING_FOR_DECISION,
        },
        data: {
      ...
      const resumed =
        (
          await tx.moduleWorkflowRun.updateMany({
            where: {
  ...
  private async ensureWorkspaceMember(
    ...
    const membership =
      await PrismaClientManager.getClient().workspaceMembership.findFirst({
        where: { workspace_id: workspaceId, user_id: userId, status: "ACTIVE" },
        select: { user_id: true },
      });
    if (!membership)
      throw new AppError(
        "Assegnatario non disponibile nel workspace.",
        "HUMAN_INTERVENTION_INVALID_ASSIGNEE",
        400,
      );
  }
