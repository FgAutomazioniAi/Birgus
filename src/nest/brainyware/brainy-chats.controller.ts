...
const createSchema = z.object({
  ...
  kind: z.enum(["AGENT", "DATABASE"]).default("AGENT"),
  agentId: z.string().uuid().optional(),
  databaseConnectionId: z.string().uuid().optional(),
});
...
export class BrainyChatsController {
  public constructor(
    @Inject(BrainyChatService) private readonly service: BrainyChatService,
  ) {}

  @Get("agents")
  @RequirePermission(PermissionKey.BRAINY_READ)
  ...
  @RequirePermission(PermissionKey.BRAINY_READ)
  public async databaseConnections(
    @CurrentRequestContext() context: RequestContext,
  ) {
    return {
      connections: await this.service.listDatabaseConnections(
        context.workspace.workspaceId,
        context.workspace.userId,
      ),
    };
  ...
  @Get("workspace-users")
  @RequirePermission(PermissionKey.BRAINY_READ)
  public async workspaceUsers(
    @CurrentRequestContext() context: RequestContext,
  ) {
    return {
      users: await this.service.listWorkspaceUsers(
        context.workspace.workspaceId,
      ),
    };
  }
  ...
  public async chats(@CurrentRequestContext() context: RequestContext) {
    return {
      chats: await this.service.listChats(
        context.workspace.workspaceId,
        context.workspace.userId,
      ),
    };
  ...
  @Post("chats")
  @RequirePermission(PermissionKey.BRAINY_WRITE)
  public create(
    @Body() body: unknown,
    @CurrentRequestContext() context: RequestContext,
  ) {
    const data = createSchema.parse(body);
    return this.service.createChat({
      workspaceId: context.workspace.workspaceId,
      userId: context.workspace.userId,
      kind: data.kind,
      workspaceAgentId: data.agentId,
      workspaceDatabaseConnectionId: data.databaseConnectionId,
  ...
  public rename(
    ...
    return this.service.renameChat({
      workspaceId: context.workspace.workspaceId,
      userId: context.workspace.userId,
      chatId,
      title: renameSchema.parse(body).title,
    });
  }

  @Patch("chats/:chatId/rag")
  @RequirePermission(PermissionKey.BRAINY_WRITE)
  public setRag(
    @Param("chatId") chatId: string,
    @Body() body: unknown,
    ...
    return this.service.setRagEnabled({
      workspaceId: context.workspace.workspaceId,
      userId: context.workspace.userId,
      chatId,
      enabled: ragSchema.parse(body).enabled,
    });
  }

  @Patch("chats/:chatId/database-memory")
  @RequirePermission(PermissionKey.BRAINY_WRITE)
  public setDatabaseMemory(
    @Param("chatId") chatId: string,
    @Body() body: unknown,
    @CurrentRequestContext() context: RequestContext,
  ) {
    return this.service.setDatabaseMemoryEnabled({
      workspaceId: context.workspace.workspaceId,
      userId: context.workspace.userId,
      chatId,
      enabled: ragSchema.parse(body).enabled,
  ...
  public shares(
    ...
    return {
      shares: this.service.listShares(
        context.workspace.workspaceId,
        context.workspace.userId,
        chatId,
      ),
  ...
  @Delete("chats/:chatId/shares/:userId")
  @HttpCode(204)
  @RequirePermission(PermissionKey.BRAINY_WRITE)
  public async unshare(
    ...
    await this.service.unshareChat({
      workspaceId: context.workspace.workspaceId,
      ownerUserId: context.workspace.userId,
      chatId,
      userId,
  ...
  public async sendStream(
    ...
    try {
      for await (const event of this.service.sendMessageStream({
        workspaceId: context.workspace.workspaceId,
        userId: context.workspace.userId,
        chatId,
        message,
  ...
  public async archive(
    ...
    await this.service.archiveChat(
      context.workspace.workspaceId,
      context.workspace.userId,
      chatId,
    );
